...
#ifdef test_prep
call write_rllsll_test_input(ncheb, npan, lmsize, nvec, nrmax, lbessel, use_sratrick1, gmatprefactor, cmoderll, cmodesll, cmodetest, hlk, jlk, hlk2, jlk2, jlk_index, rpanbound, rmesh, sll, rll, tllp, vll, alphaget)
stop
#endif
...
program test_rllsll
   ...
   read(1234, '(1es23.15)') gmatprefactor
   read(1234, '(1000es23.15)') hlk(1:lbessel,1:nrmax), jlk(1:lbessel,1:nrmax), hlk2(1:lbessel,1:nrmax), jlk2(1:lbessel,1:nrmax)
   read(1234, '(1000i9)') jlk_index(1:2*lmsize)
   read(1234, '(10000es23.15)') rmesh(1:nrmax)
   read(1234, '(10000es23.15)') rpanbound(0:npan)
   
   do ir=1, nrmax
     read(1234, '(10000es23.15)') vll(1:lmsize2, 1:lmsize2, ir)
   end do
   close(1234)
   ...
   call rllsll(rpanbound,rmesh,vll,rll,sll,tllp, &
               ncheb,npan,lmsize,lmsize2,lbessel,nrmax,nrmax, &
               nvec,jlk_index,hlk,jlk,hlk2,jlk2,gmatprefactor, &
               cmoderll,cmodesll,cmodetest,use_sratrick1,      &
               alphaget) ! lly
   ...
   if(output) then
     ...
     write(*,'(A)') '  writing output_rllslltest.txt'

     open(1234, file='output_rllslltest.txt')
     write(1234, '(A)') '# output of rllsll test rountine: sll(1:lmsize2, 1:lmsize, ir), rll(1:lmsize2, 1:lmsize, ir), tllp(1:lmsize2, 1:lmsize, ir) for ir in range(1, nrmax)'
     write(1234, '(A,3I9)') '# lmsize2, lmsize, nrmax = ', lmsize2, lmsize, nrmax
     do ir=1, nrmax
       write(1234, '(10000es23.15)') sll(1:lmsize2, 1:lmsize, ir)
       write(1234, '(10000es23.15)') rll(1:lmsize2, 1:lmsize, ir)
     end do
     write(1234, '(10000es23.15)') tllp(1:lmsize2, 1:lmsize2)
     close(1234)
...
subroutine write_rllsll_test_input(ncheb, npan, lmsize, nvec, nrmax, lbessel, use_sratrick1, gmatprefactor, cmoderll, cmodesll, cmodetest, hlk, jlk, hlk2, jlk2, jlk_index, rpanbound, rmesh, sll, rll, tllp, vll, alphaget)
   ...
   write(1234, '(1es23.15)') gmatprefactor
   write(1234, '(1000es23.15)') hlk(1:lbessel,1:nrmax), jlk(1:lbessel,1:nrmax), hlk2(1:lbessel,1:nrmax), jlk2(1:lbessel,1:nrmax)
   write(1234, '(1000i9)') jlk_index(1:2*lmsize)
   write(1234, '(10000es23.15)') rmesh(1:nrmax)
   write(1234, '(10000es23.15)') rpanbound(0:npan)
   
   do ir=1, nrmax
     write(1234, '(10000es23.15)') vll(1:nvec*lmsize, 1:nvec*lmsize, ir)
   end do
   close(1234)
