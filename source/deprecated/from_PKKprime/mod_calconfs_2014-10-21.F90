...
contains
  ...
  subroutine calc_on_fsurf_inputcard(inc,lattice,cluster,tgmatrx,nkpts,kpoints)
    ...
    !************************************************************************************* 1st level of logical switches
      ...
      if(cfg%ltorq==1 .and. cfg%lrashba==1) then
      !************************************************************************************* 2nd level of logical switches
        ...
        if(cfg%lspinperatom==1)then
          ...
          allocate( spinvec_atom(3,inc%natypd,inc%ndegen,nkpts), &
                  & STAT=ierr                                    )
          ...
          call calc_on_fsurf( inc,lattice,cluster,tgmatrx,nkpts,kpoints,                       &
                            ...
                            & torqvalue=torqval,spinvec=spinvec,spinvec_atom=spinvec_atom      )
        else!cfg%lspinperatom    
          call calc_on_fsurf( inc,lattice,cluster,tgmatrx,nkpts,kpoints,                       &
                            & fermivelocity=fermivel,spinvalue=spinval,save_eigv=cfg%saveeigv, &
                            & torqvalue=torqval,spinvec=spinvec                                )
        end if!cfg%lspinperatom
      ...
      !************************************************************************************* 2nd level of logical switches
      elseif(cfg%lrashba==1) then
      !************************************************************************************* 2nd level of logical switches

        nvector = nvector+1+inc%natypd
        allocate( spinvec(3,inc%ndegen,nkpts),                 &
!               & spinvec_atom(3,inc%ndegen,nkpts,inc%natypd), &
    ...
    !************************************************************************************* 1st level of logical switches
      ...
      !************************************************************************************* 2nd level of logical switches
        ...
        nvector = nvector+1+1+inc%natypd
        allocate(      torqval(3,inc%ndegen,nkpts),                     &
                &      spinvec(3,inc%ndegen,nkpts),                     &
                & spinvec_atom(3,inc%natypd,inc%ndegen,nkpts), STAT=ierr)
!               & spinvec_atom(3,inc%ndegen,nkpts,inc%natypd), STAT=ierr)
        if(ierr/=0) stop 'Problem allocating torqval, spinvec...'
...
!               & spinvec_atom(3,inc%ndegen,nkpts,inc%natypd),           &
    ...
    if(cfg%lrashba==1 .and. myrank==master) call save_spinvec(trim(filemode), nkpts, inc%ndegen, inc%natypd, spinvec, spinvec_atom, nsym, isym)
    ...
    if(myrank==master .and. cfg%mode==MODE_VIS .and. inc%nBZdim==3) then
      ...
      if(cfg%lrashba==1)then
        ivector = ivector+1
        vectordata(:,:,ivector) = spinvec(:,1,:)
        vectorstring(ivector) = 'Spinvec'
  ...
  subroutine calc_on_fsurf(inc,lattice,cluster,tgmatrx,nkpts,kpoints,fermivelocity,spinvalue,save_eigv,torqvalue,spinvec,spinvec_atom)
    ...
    double precision, intent(out), optional :: fermivelocity(3,nkpts),&
                                             ...
                                             & spinvec(3,inc%ndegen,cfg%nsqa,nkpts),&
                                             & spinvec_atom(3,inc%natypd,inc%ndegen,cfg%nsqa,nkpts)
...
200 FORMAT('|'$)
  ...
  subroutine calc_spinvalue_state(inc,tgmatrx,rveig_in,spin_value,eigvect_rot,torq_value,spinvec,spinvec_atom)
    ...
    use mod_spintools,  only: spin_expectationvalue, spin_crossterm, Spinrot_AlphaBeta, rotate_wavefunction, torq_expectationvalue
    ...
    double precision,   intent(out), optional :: spinvec_atom(3,inc%natypd,inc%ndegen,cfg%nsqa)

    double complex :: rveig_atom_unrot(inc%lmmaxso,inc%natypd,inc%ndegen), &
                    & rveig_atom_rot(inc%lmmaxso,inc%natypd,inc%ndegen)
    double complex :: Spin_ini(3,2), Spin_ini_atom(3,inc%natypd,2), Scross(3), Scross_atom(3,inc%natypd), Spin_final(3,inc%ndegen), Spin_final_atom(3,inc%natypd,inc%ndegen), Torq_final(3,inc%ndegen)
    ...
    if(inc%ndegen==1)then
      ...
      do isqa=1,cfg%nsqa
        ...
        if(present(spinvec)) spinvec(:,:,isqa)=Spin_final
        if(present(spinvec_atom)) spinvec_atom(:,:,:,isqa)=Spin_final_atom
      end do!isqa

      !calculate torque expectation value
      if(present(torq_value)) then
        call torq_expectationvalue(inc, inc%ndegen, tgmatrx%torq, rveig_atom_unrot, Torq_final)
...
!     write(*,'(2X,"pert e= (",2ES18.9,"), lmo= ",I0)') eigwPT(LMout), LMout
  ...
  subroutine myBcast_cfg(cfg)
    ...
    call MPI_Get_address(cfg%ltorq,        disp1(6), ierr)
    call MPI_Get_address(cfg%nsqa,         disp1(7), ierr)
    call MPI_Get_address(cfg%mode,         disp1(8), ierr)
    call MPI_Get_address(cfg%rotatemode,   disp1(9), ierr)
    call MPI_Get_address(cfg%ilayer,       disp1(10), ierr)
    call MPI_Get_address(cfg%dk_fv,        disp1(11), ierr)
    call MPI_Get_address(cfg%saveeigv,     disp1(12), ierr)
    ...
    etype1(1:10) = MPI_INTEGER
    etype1(11)   = MPI_DOUBLE_PRECISION
    etype1(12)   = MPI_LOGICAL
  ...
  subroutine save_spinvec(filemode, nkpts, ndegen, natyp, spinvec, spinvec_atom, nsym, isym)

    use mod_ioformat,   only: fmt_fn_sub_ext, ext_formatted, filename_spinvec, filename_spinvec_atom
    implicit none

    character(len=*), intent(in) :: filemode
    integer,          intent(in) :: nkpts, ndegen, nsym, isym(nsym), natyp
    double precision, intent(in) :: spinvec(3,ndegen,nkpts), spinvec_atom(3,ndegen,natyp,nkpts)
    ...
    write(filename,fmt_fn_sub_ext) filename_spinvec_atom, filemode, ext_formatted
    open(unit=326529, file=trim(filename), form='formatted', action='write')
    write(326529,'(3I8)') nkpts, nsym, ndegen, natyp
    write(326529,'(12I8)') isym
    write(326529,'(3ES25.16)') spinvec_atom
    close(326529)
  ...
  subroutine save_spinvec(filemode, nkpts, ndegen, natyp, spinvec,spinvec_atom, nsym, isym)
    ...
    character(len=*), intent(in) :: filemode
    integer,          intent(in) :: nkpts, ndegen, nsym, isym(nsym), natyp
    double precision, intent(in) :: spinvec(3,ndegen,nkpts), spinvec_atom(3,ndegen,nkpts,natyp)
