...
contains
  ...
  subroutine broyden(vector, vlen, alpha, rms, iter, n_init, broylen, mvlen)
    !
    ...
    ! interface
    integer, intent(in) :: broylen !! broylen -> number of iterations that are used for mixing
    ...
    real (kind=dp), intent(inout) :: vector(mvlen,2) !! vector(r,i) -> i=1: old vector (input), scratch (ouput)
                                                     !!             -> i=2: new vector (input), mixed vector (output)
    real (kind=dp), intent(inout) :: rms !! rms -> RMS difference between old and new vector
    ...
    real (kind=dp) :: a(broylen,broylen)
    real (kind=dp) :: b(broylen,broylen)
    real (kind=dp) :: d(broylen,broylen)
    real (kind=dp) :: cm(broylen)
    integer :: ipiv(broylen)
    ...
    if (iter<=n_init) then
      ...
      call dgetrf(nn,nn,b,broylen,ipiv,info)
      call dgetri(nn,b,broylen, ipiv, d, nn, info )
      !<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< uses lapack
