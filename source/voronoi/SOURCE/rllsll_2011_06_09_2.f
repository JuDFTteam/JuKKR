...
!           END DO !IVEC=1,NVEC
          ...
          DO IVEC=1,NVEC
            DO LM1 = 1,LMMAX
              L1 = LOFLM( LM1+LMMAX*(IVEC-1) )
              YRLL(K,LM1+LMMAX*(IVEC-1),LM1) =  TAU(K,M)*JLK(L1,K) 
              ZRLL(K,LM1+LMMAX*(IVEC-1),LM1) = -TAU(K,M)*NLK(L1,K) 
              YILL(K,LM1+LMMAX*(IVEC-1),LM1) = -TAU(K,M)*HLK(L1,K)
              ZILL(K,LM1+LMMAX*(IVEC-1),LM1) =  TAU(K,M)*JLK(L1,K)
            END DO
          END DO !IVEC=1,NVEC
        END DO
C-------------------------------------------------------
...
!       call timing_start('local3')
      ...
      DO K = 0,N
        ZSLC1SUM(K) = SLC1SUM(K) * (RINTV(M)-RINTV(M-1))/ (2.D0)
      END DO
