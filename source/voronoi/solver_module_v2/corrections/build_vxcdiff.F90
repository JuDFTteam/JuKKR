  subroutine build_vxcdiff(vxcb,magdir1,vxcdiff)
...
! contribution of xc to spin splitting
  complex(kind=c8b), intent(out) :: vxcdiff(nbmax,nbmax,lmmax,lmmax,nasusc2)
! ----------------------------------------------------------------------
  ...
  do ia2=1,nasusc2
    ...
    if (ikxc(ia) == 1 .or. ikxc(ia) == 3) then
      ...
      do i1=1,nlmsba(ia)
        ...
        vxcdiff(ib1,ib2,ilm1,ilm2,ia2) = vxcdiff(ib1,ib2,ilm1,ilm2,ia2) + sum(magdir1(:,ia)*pauli(is2,is1,1:3))*vxcb(i1,i2,ia)
      end do
      end do
    end if
  end do
! All done!
  end subroutine build_vxcdiff
