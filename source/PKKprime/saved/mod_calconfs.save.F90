module mod_calconfs
  ...
  private
  public :: calc_on_fsurf_inputcard, calc_on_fsurf, get_nsqa, calc_spinvalue_state
...
contains
  ...
  subroutine calc_on_fsurf_inputcard(inc,lattice,cluster,tgmatrx,nkpts,kpoints)
    ...
    if(myrank==master .and. cfg%lfvel==1 .and. cfg%lspin==1) then
      ...
      if(cfg%mode==MODE_INT) call calculate_spinmixing_int(inc%ndegen,cfg%nsqa,nkpts,areas,fermivel,spinval,spinmix,dos,.true.,BZVol)
      if(cfg%mode==MODE_VIS) call calculate_spinmixing_vis(inc%ndegen,cfg%nsqa,nkpts,nkpts_all,kpt2irr,kpoints,fermivel,spinval,spinmix,dos,.true.,BZVol)

    elseif(myrank==master .and. cfg%lfvel==1 .and. cfg%lspin==0) then

      if(cfg%mode==MODE_INT) call calculate_dos_int(nkpts,areas,fermivel,dos,.true.,BZVol)
      if(cfg%mode==MODE_VIS) call calculate_dos_vis(nkpts,nkpts_all,kpt2irr,kpoints,fermivel,dos,.true.,BZVol)

    end if


    !output visualizationdata
    if(myrank==master .and. cfg%mode==MODE_VIS) then
...
!   double complex,   intent(out), optional :: eigenvectors(inc%lmmaxso,inc%natypd,inc%ndegen,cfg%nsqa,nkpts)
    ...
    !save the fermi velocity to a file
    if(present(fermivelocity))then
#ifdef CPP_MPI
...
#endif
!     write(*,*) 'end of fermivelocity'
    end if!present(fermivelocity)


    !save the spinvalue to a file
    if(present(spinvalue))then
#ifdef CPP_MPI
      !gather the fermi-velocity from the processes
...
!     write(*,'(2X,"pert e= (",2ES18.9,"), lmo= ",I0)') eigwPT(LMout), LMout
  ...
  subroutine calculate_spinmixing_int(ndeg,nsqa,nkpts,areas,fermivel,spinval,spinmix,dos,printout,BZVol)
    use mpi
    use mod_mympi, only: myrank, master
    implicit none

    integer,          intent(in)  :: ndeg, nsqa, nkpts
    ...
    if(printout .and. myrank==master)then
      write(*,'(A,ES25.16,A)') "DOS(E_F)=", dos, " [unnormalized]"
      if(present(BZVol)) write(*,'(A,ES25.16,A)') "DOS(E_F)=", dos/BZVol, " [normalized]"
      do isqa=1,nsqa
        write(*,'(A,I4,3(A,ES25.16))') "Elliott-Yafet parameter: isqa= ", isqa,", b^2= ", integ(isqa)," [unnormalized], ", spinmix(isqa), " [normalized by DOS]"
      end do!isqa
    end if!printout
  ...
  subroutine calculate_dos_int(nkpts,areas,fermivel,dos,printout,BZVol)
    use mpi
    use mod_mympi, only: myrank, master
    implicit none
  ...
  subroutine calculate_spinmixing_vis(ndeg,nsqa,nkpts,nkpts_all,kpt2irr,kpoints,fermivel,spinval,spinmix,dos,printout,BZVol)
    use mpi
    use mod_mympi, only: myrank, master
    use mod_mathtools, only: crossprod, simple_integration
    implicit none

    integer,          intent(in)  :: ndeg, nsqa, nkpts, nkpts_all, kpt2irr(nkpts_all)
  ...
  subroutine calculate_dos_vis(nkpts,nkpts_all,kpt2irr,kpoints,fermivel,dos,printout,BZVol)
    use mod_mympi, only: myrank, master
    use mod_mathtools, only: crossprod, simple_integration
    use mpi
    implicit none

    integer,          intent(in)  :: nkpts, nkpts_all, kpt2irr(nkpts_all)
