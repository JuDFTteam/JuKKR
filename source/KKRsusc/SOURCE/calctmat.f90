...
SUBROUTINE CALCTMAT(ERYD,VPOT,CELL,ZATOM,LMAXATOM,TMATLL,config,ispin,nspin,iatom,inpsusc)!     & ! susc |||| 'iatom,inpsusc' added by Benedikt (2014/06)
...
!   use configparams, only: icst,ins,nsra
  ...
  use type_cell
  use type_config
  use mod_config, only: config_testflag, config_runflag ! susc |||| config_runflag added by Benedikt (2014/06)
...
!interface variables
  ...
  integer,intent(in)                        ::  iatom   ! susc |||| added by Benedikt (2014/06)
...
!                                                 s(0:lmaxatom)
  ...
  integer                                   :: nrmaxd    ! susc |||| added by Benedikt (2014/06)
...
! Fill pzsusc array, etc. with radial wave functions for energy:    ! susc |||| added by Benedikt (2014/06)
if ( config_runflag('kkrsusc') ) then                               ! susc |||| added by Benedikt (2014/06)
  inpsusc%pzsusc(1:cell%nrmax,0:lmaxatom,iatom,ispin) = pz          ! susc |||| added by Benedikt (2014/06)
  inpsusc%fzsusc(1:cell%nrmax,0:lmaxatom,iatom,ispin) = fz          ! susc |||| added by Benedikt (2014/06)
  inpsusc%qzsusc(1:cell%nrmax,0:lmaxatom,iatom,ispin) = qz          ! susc |||| added by Benedikt (2014/06)
  inpsusc%szsusc(1:cell%nrmax,0:lmaxatom,iatom,ispin) = sz          ! susc |||| added by Benedikt (2014/06)
...
! we pass the t-matrix to the module kkrsusc
if ( config_runflag('kkrsusc') ) inpsusc%tmatll(:,:,iatom) = tmatll(:,:)           ! susc |||| added by M&J (2015/06)
