      SUBROUTINE FORCE(FLM,FLMC,LMAX,NSPIN,NSTART,NEND,VPOT,
     +                 IRWS,IRMD)
      IMPLICIT NONE
c-----------------------------------------------------------------------
...
C     .. Local Scalars ..
      ...
      INTEGER I,IATOM,IPOT,IRWS1,ISPIN,LM,M
C     ..
C     .. Local Arrays ..
...
c
                  V1(I) = DENSITY(IATOM)%RHOC(I,ISPIN,IATOM)* (2.0D0*VPOT(I,LM,ISPIN,IATOM)/CELL(IATOM)RMESH(I)+ &
                          DV)/ (4.0D0*PI) + V1(I)
               END DO !I
c
               ...
               V1(IRWS1-1) = DENSITY(IATOM)%RHOC(IRWS1-1,ISPIN,IATOM)* &
...
c
               V1(IRWS1) = DENSITY(IATOM)%RHOC(IRWS1,IPOT)* &
