  subroutine rymy(uvec,nl,nlm,rylm)
...
! +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
! loop over m values
  dfac = 1.d0; root = 1.d0; sthm = 1.d0; p(0,0) = invr4pi
  do m=0,nl
    if (m > 0) then
      sthm = sthm*sth
!     there should be a minus sign here for Condon-Shortley
      dfac = -dfac*sqrt((2.d0*m-1.d0)**2/(2.d0*m*(2.d0*m-1.d0)))
      root = sqrt((2.d0*m+1.d0))*invr4pi
      p(m,m) = dfac*root*sthm
    end if
    if (m < nl) p(m+1,m) = sqrt(2.d0*m+3.d0)*cth*p(m,m)
! upward recursion in l
    do l=m+2,nl
      root1 = sqrt((2.d0*l+1)/(l+m))*sqrt((2.d0*l-1)/(l-m))
      root2 = sqrt((l-1.d0+m)/(2.d0*(l-1)+1.d0))*sqrt((l-1.d0-m)/(2.d0*(l-1)-1.d0))
...
! +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  ...
  do l=0,nl
    ...
    rylm(i) = p(l,0)  ! m = 0
    do m=1,l
      rylm(i+m) = rtwo*p(l,m)*c(m)  ! m > 0 cosine type
      rylm(i-m) = rtwo*p(l,m)*s(m)  ! m < 0 sine type
    end do
    i = i + l
  end do
! All done!
