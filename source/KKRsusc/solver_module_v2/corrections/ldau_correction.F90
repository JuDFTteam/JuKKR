  subroutine ldau_correction()
! LDA+U correction to the GF
  use global
...
! SOC potential
  complex(kind=c8b) :: vldau(nrmax), vldaub(nlmsb,nlmsb)
! auxiliary
  complex(kind=c8b) :: work1(nlmsb,nlmsb), work2(nalms,nalms)
  complex(kind=c8b) :: gfpqldau(nlmsb,nlmsb)
  complex(kind=c8b) :: pzlldau(nlmsb,nlms), pzrldau(nlmsb,nlms)
...
! loop over energies
  do ie=1,nesusc
    write(iodb,*) "LDA+U correction ie=", ie
! loop over atoms
    do ia=1,nasusc
!   construct SOC potential
      edummy = escf(ie)
!      edummy = 0.d0
!      call build_vldau(ia,c,edummy,vldau)
!   compute the matrix elements in the basis
      call build_vldaub(ia,vldaub)
!   ********************************************************************
!              solve Lippmann-Schwinger and Dyson equations
!   ********************************************************************
!     --> construct 1 - G0.Vlda+u
      work1 = 0.d0
      do i=1,nlmsba(ia)
        work1(i,i) = 1.d0
      end do
      call zgemm('N','N',nlmsba(ia),nlmsba(ia),nlmsba(ia),cminus,gfpq(:,:,ia,ie),nlmsb,vldaub,nlmsb,cplus,work1,nlmsb)
!     --> LU decomposition of 1 - G0.Vlda+u
      call zgetrf(nlmsba(ia),nlmsba(ia),work1,nlmsb,ipiv,info)
      if (info /= 0) then
        write(*,*) "LU fail 1 - G0.Vldau at ie,ia=", ie, ia, info
        stop
      end if
!     --> new single-site GF: (1 - G0.Vlda+u).G = G0
      gfpqldau = gfpq(:,:,ia,ie)
      call zgetrs('N',nlmsba(ia),nlmsba(ia),work1,nlmsb,ipiv,gfpqldau,nlmsb,info)
      if (info /= 0) then
        write(*,*) "LDA+U single-site GF failed at ie,ia=", ie, ia
        stop
      end if
!     --> solve Lipp-Schw for rhs wfn: (1 - G0.Vlda+u).R^r = R0^r
!       nlms are the boundary conditions
      pzrldau = pzr(:,:,ia,ie)  ! the initial rhs solution is needed in t^l
      call zgetrs('N',nlmsba(ia),nlms,work1,nlmsb,ipiv,pzrldau,nlmsb,info)
      if (info /= 0) then
        write(*,*) "LDA+U rhs Lipp Schw failed at ie,ia=", ie, ia
        stop
      end if
...
!      write(iodb,'("maxelem  ",es14.6)') maxelem
      do j=1,nlms
        do i=1,nlmsba(ia)
          if (abs(pzrldau(i,j)) > ldautol*maxelem) then
...
!     --> change in t-matrix: t^r = R0^l.Vldau.R^r
      tmatr = 0.d0; work1 = 0.d0
!     Vldau.R^r -> work
      call zgemm('N','N',nlmsba(ia),nlms,nlmsba(ia),cplus,vldaub,nlmsb,pzrldau,nlmsb,czero,work1,nlmsb)
!     R0^l.work -> t^r
...
!      end do
!   ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
!     --> construct 1 - G0^T.Vldau^T
      work1 = 0.d0
      do i=1,nlmsba(ia)
        work1(i,i) = 1.d0
      end do
      call zgemm('T','T',nlmsba(ia),nlmsba(ia),nlmsba(ia),cminus,gfpq(:,:,ia,ie),nlmsb,vldaub,nlmsb,cplus,work1,nlmsb)
!     --> LU decomposition of 1 - G0^T.Vldau^T
      call zgetrf(nlmsba(ia),nlmsba(ia),work1,nlmsb,ipiv,info)
      if (info /= 0) then
        write(*,*) "LU fail 1 - G0^T.Vldau^T at ie,ia=", ie, ia, info
        stop
      end if
!     --> solve Lipp-Schw for left hand side wfn: (1 - G0^T.Vldau^T).R^l = R0^l
!       nlms are the boundary conditions
      pzlldau = pzl(:,:,ia,ie)
      call zgetrs('N',nlmsba(ia),nlms,work1,nlmsb,ipiv,pzlldau,nlmsb,info)
      if (info /= 0) then
...
!      write(iodb,'("maxelem  ",es14.6)') maxelem
      do j=1,nlms
        do i=1,nlmsba(ia)
          if (abs(pzlldau(i,j)) > ldautol*maxelem) then
...
!     --> change in t-matrix: t^l = R^l.Vldau.R0^r
      tmatl = 0.d0; work1 = 0.d0
!     R^l.Vldau -> work
      call zgemm('T','N',nlms,nlmsba(ia),nlmsba(ia),cplus,pzlldau,nlmsb,vldaub,nlmsb,czero,work1,nlmsb)
!     work.R0^r -> t^l
...
!     --> solve Dyson equation
    ...
    if (info /= 0) then
      write(*,*) "LDA+U struct Dyson failed at ie=", ie
      stop
    end if
  end do
! Flag SOC as applied
!  ldau_applied = .true.
  ...
  write(*,'(/," LDA+U correction time=",f10.3," s",/)') finish - start
! All done!
  end subroutine ldau_correction
