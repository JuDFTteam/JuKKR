  subroutine spinrot_gf(magdir0,magdir1)
...
!    write(*,'(i4," rot sx ",8f8.4)') ia, matmul(spinrot(:,:,ia),matmul(pauli(:,:,1),conjg(transpose(spinrot(:,:,ia)))))
!    write(*,'(i4," rot sy ",8f8.4)') ia, matmul(spinrot(:,:,ia),matmul(pauli(:,:,2),conjg(transpose(spinrot(:,:,ia)))))
!    write(*,'(i4," rot sz ",8f8.4)') ia, matmul(spinrot(:,:,ia),matmul(pauli(:,:,3),conjg(transpose(spinrot(:,:,ia)))))
  end do
  call cpu_time(start)
! **************
  do ie=1,nesusc
...
!           selection rules
            if (ilm == jlm .and. pb == qb .and. plm == qlm) then
