  subroutine groundstate_fit()
! Ground state properties, using fit to GF
...
! Get fitting coefficients
  ...
  do ja=1,nasusc
    do ia=1,nasusc
!      call ratfit_gf2(ia,ja,.true.,.true.)
      call ratfit_gf3(ia,ja,.true.,.true.)
!      call ratsolve_gf(ia,ja,.true.,.true.)
    end do
  end do
...
! fill up new rho2ns
    new_rho2ns(:,:,:,ia) = 0.d0
    new_dir = (/0.d0,0.d0,1.d0/)
    do klm=1,lmmax2
      do jlm=1,lmmax
      do ilm=1,lmmax
        fac = gaunt(ilm,jlm,klm)
...
! All done
  end subroutine groundstate_fit
