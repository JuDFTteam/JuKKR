...
#include "macros.h"
  ...
  subroutine createRadialMeshData(self, irmd, ipand, meshn, nfu)
    ...
    integer, intent(in), optional :: meshn, nfu
...
!===========  Index file ======================================================
  ...
  subroutine writeRadialMeshDataIndexDA(self, fileunit, recnr, max_reclen)
    ...
    FILEWRITE (fileunit, rec=recnr) self%irmd, self%ipand, &
#ifdef USE_OLD_MESH
                                self%meshn, self%NFU, &
#endif
                                max_reclen, MAGIC_NUMBER + recnr
  ...
  subroutine openRadialMeshDataIndexDAFile(self, fileunit, filename)
    ...
    max_reclen = 0
    inquire (iolength = reclen) self%irmd, self%ipand, &
#ifdef USE_OLD_MESH
                                self%meshn, self%NFU, &
#endif
                                max_reclen, MAGIC_NUMBER
...
!================= private helper functions ===================================
  subroutine readRadialMeshDataHeader(self, fileunit, recnr, irmd, ipand, max_reclen, meshn, nfu)
    ...
    integer, intent(out) :: ipand
    integer, intent(out) :: max_reclen
    integer, intent(out), optional :: meshn, nfu

    integer :: magic, checkmagic

    checkmagic = MAGIC_NUMBER + recnr

    FILEREAD  (fileunit, rec=recnr) irmd, ipand, &
#ifdef USE_OLD_MESH                                
                                meshn, nfu, &
#endif
                                max_reclen, magic
