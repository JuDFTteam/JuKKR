...
!-------------------------------------------------------------------------------
  ...
  subroutine rhoin(ar, cden, cr, df, gmat, ek, rho2ns, irc1, nsra, efac, pz,  &
    ...
    integer, intent(in) :: jend((2*lmax+1)**2,0:lmax,0:lmax), iend
  ...
  subroutine rhons(den, df, drdi, gmat, ek, rho2ns, ipan, ircut, thetas, &
    ...
    integer, intent(in) :: iend, ipan, nsra
    !     ..
    !     .. array arguments ..
    !     double complex ar(lmmaxd,lmmaxd),cr(lmmaxd,lmmaxd),den(0:lmaxd1),ekl(0:lmaxd),fz(irmd,0:lmaxd),gmat(lmmaxd,lmmaxd), &
    !       pns(lmmaxd,lmmaxd,irmind:irmd,2),pz(irmd,0:lmaxd),qns(lmmaxd,lmmaxd,irmind:irmd,2),qz(irmd,0:lmaxd),sz(irmd,0:lmaxd)
    !     double precision cleb(*),drdi(irmd),rho2ns(irmd,lmpotd),thetas(irid,nfund)
    !     integer icleb(ncleb,3),ifunm(*),ircut(0:ipand),jend(lmpotd,0:lmaxd,0:lmaxd),lmsp(*)

    double complex, intent(in) :: ar((lmax+1)**2,(lmax+1)**2), cr((lmax+1)**2,(lmax+1)**2)
    double complex, intent(out) :: den(0:lmax+1)
    double complex, intent(in) :: ekl(0:lmax)
    double complex, intent(in) :: fz(irmd,0:lmax)
    double complex, intent(in) :: gmat((lmax+1)**2,(lmax+1)**2)
    double complex, intent(in) :: pns((lmax+1)**2,(lmax+1)**2,irmd-irnsd:irmd,2)
    double complex, intent(in) :: pz(irmd,0:lmax)
    double complex, intent(in) :: qns((lmax+1)**2,(lmax+1)**2,irmd-irnsd:irmd,2)
    double complex, intent(in) :: qz(irmd,0:lmax)
    double complex, intent(in) :: sz(irmd,0:lmax)
    double precision, intent(in) :: cleb(*)
    double precision, intent(in) :: drdi(irmd)
  ...
  subroutine rhoval(ldorhoef,icst,ielast,nsra, &
    ...
    double complex, intent(out) :: den(0:lmaxd+1,iemxd)
    ...
    integer :: lmmaxd, lmaxd1, nspind, lmpotd, ist
    ...
    allocate(pns(lmmaxd,lmmaxd,irmd-irnsd:irmd,2), qns(lmmaxd,lmmaxd,irmd-irnsd:irmd,2), stat=ist)
    if (ist /= 0) then
    ...
    deallocate(pns, qns, stat=ist) ! ignore status
