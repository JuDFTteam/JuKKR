...
function energy_electrostatic_wrapper(v_potential, Z_nuclear, RHO2NS, shgaunts, atomdata) result(ecou)
  ...
  ecou = energy_electrostatic(lpot, lmax_potential, &
...
!------------------------------------------------------------------------------
!>    Modified ecoub to calculate electrostatic energies.
!>
...
!--->     now integrate
!
  call simpk(er,ecou_temp,ipan1,ircut, drdi)
