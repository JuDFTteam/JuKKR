...
C
      INTEGER INFO(4), NDIM, NLEN, NLIM, LM2,IAT,ITCOUNT,IE,K,I
...
C
      LOGICAL NOISE_I
      INTEGER INIT, REVCOM, TIME, MTIME, RTIME
      REAL    RAND
      DOUBLE COMPLEX ZTMP
...
C
      IF (VF.NE.0) 
     + WRITE (VF,'(A3,2I8,A3,3I8,2E11.4)') 'IE=',IE,K,
     +                                 'LM=',LM2,0,0,DONE,DONE
      IF (TF.NE.0) 
     + WRITE (TF,'(A3,2I8,A3,3I8,2E11.4)') 'IE=',IE,K,
     +                                 'LM=',LM2,0,0,DONE,DONE
C
...
C
C
      INQUIRE(FILE='NOISE_I',EXIST=NOISE_I)
C
C
      IF (NOISE_I) THEN
        CALL SYSTEM_CLOCK(TIME,RTIME,MTIME)
        CALL SRAND(TIME)
        DO I=1,NLEN
          VECS(I,1) = 
     +    DCMPLX((RAND()-0.5)*TOL*1D-4,(RAND()-0.5)*TOL*1D-4)
        ENDDO
      ELSE
C
C
