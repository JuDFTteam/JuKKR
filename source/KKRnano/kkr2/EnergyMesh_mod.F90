...
!   use GaussWeights_mod, only: 
    ...
    double precision, parameter :: pi=3.14159265358979312d0, kb=0.6333659d-5, ryd=13.6058d0
    double precision wi(228),xi(228)
!     ..
!     .. external subroutines ..
    logical, external :: opt
    external :: gaufd, gauleg
    ...
    F9090="(5X,'GF integration rectangular contour (ImE > 0)',/,5X,'Number of energy points :',I4,13X,'poles =',I2,/,23X,'contour: N1 =',I2,', N2 =',I4,', N3 =',I2)", &
    F9091="(5X,'GF integration rectangular contour (ImE < 0)',/,5X,'Number of energy points :',I4,13X,'poles =',I2,/,23X,'contour: N1 =',I2,', N2 =',I4,', N3 =',I2)", &
...
! ------------------------------------------------------------- npol = 0
    if (npol == 0) then
        open(87, file='FGRID', form='formatted', status='old', action='read', iostat=i)
        if (i == 0) then
          ! file fgrid exists
          read(87,*) febot, femu
          close(87)
          ...
          do i = 2, npnt2-1
            npnt = npnt + 1
            if (npnt+1 > iemxd) then
              write(6,'(/,5x,a,i4)') 'dimension error: increase iemxd in inc.p to at least ',npnt
              stop '     < emesht >'
            endif
            er = febot + (i-1)*de
...
!           if (opt('gf-ef   ')) ez(npnt) = emu + npol*dcmplx(0.d0,etk)
        ...
        do i = npnt3, 1, -1
          ...
          df(npnt) = -wi(i)*de
        enddo ! i
        write(6, fmt=F9091) npnt, -npol, npnt1, npnt2, npnt3
    endif
! ======================================================================
    write(6,*)
