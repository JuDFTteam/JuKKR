...
#ifndef NUMBERC
! defaults
#define NUMBERC integer
#define NUMBERMPIC MPI_INTEGER
#endif
...
!     module procedure comm_gatherC
!   endinterface
! 
!   interface comm_redistribute
!     module procedure comm_redistributeC, &
!                      comm_redistributeVC
!   endinterface
!   
!   interface comm_bcast
!     module procedure comm_bcastC, &
!                      comm_bcast2C
!   endinterface
! 
!   interface send_array
!     module procedure send_arrayC
!   endinterface
  
  ! deprecated public statements (to be private in the future)
  public :: comm_gatherC
  public :: comm_redistributeC
  public :: comm_redistributeVC
  public :: comm_bcastC
  public :: comm_bcast2C
  public :: send_arrayC
  ...
  subroutine comm_gatherC(my_world_rank, array, blocksize, owning_ranks, receiver)
    integer, intent(in) :: my_world_rank
    NUMBERC, intent(inout) :: array(*)
    ...
    do rank_index = 1, size(owning_ranks)
      ...
      call send_arrayC(my_world_rank, array(start), blocksize, sender, receiver)

    enddo ! rank_index

  endsubroutine comm_gatherC

  !--------------------------------------------------------------------------
  !> Redistributes array-parts among groups of ranks.
  subroutine comm_redistributeC(my_world_rank, array, blocksize, old_owners, new_owners)
    integer, intent(in) :: my_world_rank
    NUMBERC, intent(inout) :: array(*)
    integer, intent(in) :: blocksize
  ...
  endsubroutine comm_redistributeC
  ...
  subroutine comm_redistributeVC(my_world_rank, array, blocksizes, old_owners, new_owners)
    integer, intent(in) :: my_world_rank
    NUMBERC, intent(inout) :: array(*)
    ...
    do rank_index = 1, size(old_owners)
      ...
      call send_arrayC(my_world_rank, array(start), blocksizes(rank_index), sender, receiver)
      start = start + blocksizes(rank_index)

    enddo ! rank_index

  endsubroutine comm_redistributeVC
  ...
  subroutine comm_bcastC(my_world_rank, array, length, ranks, owner)
    integer, intent(in) :: my_world_rank
    NUMBERC, intent(inout) :: array(*)
    ...
    call send_arrayC(my_world_rank, array, length, sender, receiver)
  ...
  !> Also accepts single entry in ranks
  !> Also accepts duplicate entries - but: unnecessary communication
  subroutine comm_bcast2C(my_world_rank, array, length, ranks)
    integer, intent(in) :: my_world_rank
    NUMBERC, intent(inout) :: array(*)
    integer, intent(in) :: length
    integer, intent(in) :: ranks(:)
  ...
  endsubroutine comm_bcast2C

  !-----------------------------------------------------------------
  !> Helper routine. Sends 'length' entries of 'array' from sender to
  !> receiver (ranks in MPI_COMM_WORLD).
  subroutine send_arrayC(my_world_rank, array, length, sender, receiver)
    integer, intent(in) :: my_world_rank
    NUMBERC, intent(inout) :: array(*)
    integer, intent(in) :: length
    ...
    if (sender /= receiver) then
      ...
      if (my_world_rank == sender) then
        call MPI_Send(array, length, NUMBERMPIC, receiver, tag, MPI_COMM_WORLD, ierr)
        COMMCHECK(ierr)
      endif

      if (my_world_rank == receiver) then
        call MPI_Recv(array, length, NUMBERMPIC, sender, tag, MPI_COMM_WORLD, status, ierr)
        COMMCHECK(ierr)
      endif

    endif

  endsubroutine send_arrayC

endmodule comm_patternsC_mod
