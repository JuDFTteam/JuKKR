...
!       chunk_inds(2,ii) = atom_requested - num_local_atoms*chunk_inds(1,ii) ! local index
  ...
  endsubroutine copyFromI_com



  subroutine exposeBufferI(win, buffer, bsize, chunk_size, comm)
...
!     type(ChunkIndex), intent(in) :: chunk_inds(:)
    ...
    call fenceI(win)
    call copyChunksNoSyncI(dest_buffer, win, chunk_inds, chunk_size)
    ! ensure that Get has completed and dest_buffer is valid
    call fenceI(win)

  endsubroutine copyChunksI
...
!       local_ind  = chunk_inds(ii)%local_ind
  ...
  endsubroutine copyChunksNoSyncI

  !------------------------------------------------------------------------------
  !> Wrapper for fence call.
  subroutine fenceI(win)
    integer, intent(inout) :: win
  ...
  endsubroutine hideBufferI

endmodule one_sided_commI_mod
