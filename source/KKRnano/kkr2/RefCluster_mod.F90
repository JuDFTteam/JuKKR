...
module RefCluster_mod
  ...
  type RefCluster
    ...
    integer :: atom_id !< basis atom index of central cluster atom
    !> reference to LatticeVectors datastructure
    integer(kind=4), allocatable :: atom(:)  !< dim(nacls) basis atom indices of cluster atoms
  ...
  subroutine createRefCluster(self, lattice_vectors, rbasis, rcut, global_atom_id)
    ...
    self%atom_id = global_atom_id

    call clsgen99(global_atom_id, rcut, lattice_vectors, rbasis, self%nacls, self%rcls, self%atom, self%ezoa, self%numn0, self%indn0)
#ifdef DEBUG
  ...
  subroutine clsgen99(atom_id, rcut, rr, rbasis, nacls, rcls, atom, ezoa, numn0, indn0)
    ...
    ! atom.  
    !
    integer, intent(in) :: atom_id !< central atom index atom into rbasis(1:3,:)
    double precision, intent(in) :: rcut
    ...
    integer :: ia, ib, iat, n, ist, macls ! macls==naez*nr
    logical :: couplmat
...
program test_clusters
  ...
  double precision :: bravais(3,3), rbasis(3,4), dist2, rcut = 1.5
  ...
  do ii = 1, ref_cluster%nacls
    ...
    if (dist2 > rcut*rcut) stop "ERROR in cluster generation."
  enddo ! ii
