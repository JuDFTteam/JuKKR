...
%	\item KKRnano
%	\item DFT
%	\item Magnetic textures in B20-MnGe (helical spiral, hedgehog lattice, Bloch point)
...
The code allows us to treat system sizes of up to half a million atoms per unit cell and to
simulate a non-collinear alignment of atomic spins.
This capibility is used to investigate nanometer-sized magnetic textures in the germanide B20-\ce{MnGe},
a material that is potentially going to play an important role in future spintronic devices.
A subsequent performance analysis of KKRnano on Hazel Hen emphasizes the good scaling behaviour with increasing
system size and demonstrates the extensive integration of highly optimized libraries.
%the parallelization concept of KKRnano is discussed and
%The electronic structure code, KKRnano, 
%specifically designed for petaFLOP computing. 
%Our method falls in the category of codes that scale linearly with the number of atoms,
%so that we can realize system sizes of up to half a million atoms in a unit cell.
%We have developed a unique electronic structure code, KKRnano, specifically designed for petaFLOP computing.
%Our method scales linearly with the number of atoms, so that we can realize system sizes of up to half
%a million atoms in a unit cell. Recently, we implemented a relativistic generalization of the algorithm
%enabling the calculation of complex non-collinear magnetic structures in real space.
%The new version of KKRnano is used to investigate the magnetic B20 germanides.
%B20-\ce{MnGe} and B20-\ce{FeGe} 
%as well as alloys of B20-$\ce{Mn}_{1-x}\ce{Fe}_{x}\ce{Ge}$ type with varied proportions of Mn and Ge. 
%These compounds are well known for their exhibition of helical states.
%The focus of our recent work is on B20-\ce{MnGe}. Here, the observation 
%of topologically protected magnetic particles,
%also known as skyrmions, was recently reported which
%makes it a promising candidate for future spintronic devices.
}
...
\begin{figure}[htb]
	...
	\hfill
	\subfloat[]{\includegraphics[width=.45\textwidth]{Figures/helicalspiral.png}\label{fig:mnge_spiral}}
	\hfill
...
We commenceour investigations by setting up a 6x6x6 B20-\ce{MnGe} supercell (1728 atoms), where we use PBEsol 
