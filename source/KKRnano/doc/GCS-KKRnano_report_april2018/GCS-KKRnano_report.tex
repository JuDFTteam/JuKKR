...
%\begin{itemize}
%	\item An advantage of the way spin-orbit coupling (SOC) is implemented in KKRnano is that
%		its contribution is added to the scalar-relativistic
%		potential in a perturbation-like manner. This allows to scale its strength so that
%		SOC can artificially be made stronger.
%	\item We perform a comparison of the Bloch point (BP) state and the ferromagnetic (FM) state
%	\item 6x6x6 supercell with 2x2x2 k-point-mesh and LDA functional
%	\item We allow the magnetic moments to relax during the convergence process.
%		This leads to a small canting of the moments which is a known effect in
%		B20 materials \cite{chizhikov_multishell_2013}
%	\item A series of caluclations is conducted ranging from an SOC that is not altered to
%		an enhancement of the SOC potential by a
%		factor of $f_{\text{SOC}}=4.0$ (see \Cref{fig:MnGe_kkrnano_comparison_bp}).
%	\item As expected, the BP state is energetically not preferred over the FM state for a small
%		scaling of SOC.
%	\item When SOC is increased by $f_{\text{SOC}}=3.5$,
%		both states are energetically more or less equivalent.
%	\item Above 3.5 KKRnano clearly prefers the BP state over the FM state with
%		an energy difference of up to 4 meV/f.u.
%	\item The most beneficial scaling value is found to be $f_{\text{SOC}}=4.0$.
%	\item The effect of SOC scaling on the magnetic moment can be deemed negligible. Over the whole
%		 range it decreases by 0.07 $\mu_{B}$ for each of the two states.
%	 \item It would be interesting to check whether a global minimum of $E_{\text{BP}}-E_{\text{FM}}$ can
...
%	 \item However, this is not possible as our method becomes increasingly numerically unstable for 
...
both states are energetically more or less equivalent.
Above 3.5 KKRnano clearly prefers the BP state over the FM state with
an energy difference of up to 4 meV/f.u.
...
\end{center}
	...
	The runtimes are divied into four subgroups: BLAS for library-based linear algebra operations, 
	ETC for ??, USER for ?? and MPI contains all the time that is spent on communication and synchronization
	measures between the tasks.
	}
