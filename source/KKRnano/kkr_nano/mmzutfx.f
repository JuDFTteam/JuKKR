...
C**********************************************************************
C
      SUBROUTINE MMZUTFX(NDIM,NLEN,NLIM,VECS,TOL,INFO,RETLBLLM,
     +                   IERR,N,TF,TRES,VF,
     +                   ALPHA,BETA,ETA,RHO,
     +                   COS,VAR,R0,RESN,TAU,UCHK,UNRM )
...
C
      INTEGER INFO(4), NDIM, NLEN, NLIM, I, DIMVEC
...
C
      INTEGER IERR,N,RETLBLLM,TF,TRES,VF
      DOUBLE COMPLEX ALPHA,BETA,ETA,RHO
      DOUBLE PRECISION COS,VAR,R0,RESN,TAU, 
     + UCHK,UNRM
C      SAVE             COS, VAR, R0, RESN, TAU, UCHK, UNRM
C
...
C
      ...
      IF (REVCOM.EQ.0) THEN
         N = 0
         IF (RETLBLLM.EQ.0) GO TO 10
      ELSE IF (REVCOM.EQ.1) THEN
         IF (RETLBLLM.EQ.30) THEN
            GO TO 30
         ELSE IF (RETLBLLM.EQ.40) THEN
            GO TO 40
         ELSE IF (RETLBLLM.EQ.60) THEN
            GO TO 60
         ELSE IF (RETLBLLM.EQ.70) THEN
            GO TO 70
         END IF
      END IF
      IERR = 1
