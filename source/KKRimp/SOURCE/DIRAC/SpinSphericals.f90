...
!  ComplexSphericalHarmonic = AssociatedLegendrePolynomial(l,abs(m),cos(theta))
  ComplexSphericalHarmonic = sqrt(((2.0d0*l+1.0d0)*factorial(l-abs(m))) &
     /(4*Pi*factorial(l+abs(m)))) * exp(i*m*phi) * AssociatedLegendrePolynomial(l,m,cos(theta))
...
implicit none
  ...
  if(m == 0) then
      ...
      RealSphericalHarmonic = 1.0d0/sqrt(2.0d0) *(  ComplexSphericalHarmonic(l, m,theta,phi) &
                                                   +ComplexSphericalHarmonic(l,-m,theta,phi) )
  else if (m<0) then
      RealSphericalHarmonic = i/sqrt(2.0d0)     *(  ComplexSphericalHarmonic(l,-m,theta,phi) &
                                                   -ComplexSphericalHarmonic(l, m,theta,phi) )
  end if
