...
!       PARAMETER (LMPOTD= (LPOTD+1)**2)
      INTEGER LMXSPD
      INTEGER IRMD,NFUND,IRID,NGSHD,IPAND
...
c test fivos gga start
      do ir = 1,irmd        ! test fivos gga
         write(78,7844) r(ir),DRRL(ir,1),DDRRL(ir,1) ! test fivos gga
     &        ,DRRUL(ir,1),DDRRUL(ir,1) ! test fivos gga
      enddo ! test fivos gga
 7844  format(10e16.7)! test fivos gga

c test fivos gga end
...
c
  ...
  190         CONTINUE
  200       CONTINUE
            CALL SIMPK(ER(1,L),EXC(L,IATYP),IPAN1,IRCUT,DRDI,IPAND)
  210     CONTINUE
        END IF
