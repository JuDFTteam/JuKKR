c 20.09.95 ***************************************************************
      SUBROUTINE INTOUT(G,F,V,E,L,NNE,K2,DG,A,B,Z,NONSRA)
c ************************************************************************
c
C     .. Scalar Arguments ..
      DOUBLE PRECISION A,B,DG,E,Z
      INTEGER K2,L,NNE,NONSRA
C     ..
C     .. Array Arguments ..
      DOUBLE PRECISION F(*),G(*),V(*)
C     ..
C     .. Local Scalars ..
      DOUBLE PRECISION AA,ALFA,B1,B2,BB,BETA,C,DET,DF1,DF2,DF3,DG1,DG2,
     +                 DG3,DR,EA,FLLP1,H83,P12,P21,PHI,PP,QQ,R,R1,R2,R3,
...
C     ..
      ...
      PX(1) = 0.D0
      QX(1) = 0.D0
      IF (Z.LE.20.D0 .OR. NONSRA.EQ.1) THEN
        S = 1.D0*L
        PX(2) = 0.D0
        PX(3) = 1.D0
        DO 10 K = 2,9
   ...
   40   CONTINUE
        G(K) = (R**S)*PP
        F(K) = (R**S)*QQ
c        if(g(k)*g(k-1).lt.0.D0) nne=nne+1
        SG = DSIGN(1.0D0,G(K))
