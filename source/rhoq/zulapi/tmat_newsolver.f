     ...
     +                        NCHEB,R_LOG,
     +                        VINS,VM2Z,PNS_SO,TMATLL,THETA,PHI,I1,
     +                        IMPLAYER,TSTAR,DRDI)
       IMPLICIT NONE
...
c-----------------------------------------------------------------
     ...
     +                                RLLTEMP(:,:),PNSTEMP(:,:)
       ...
       DOUBLE COMPLEX, ALLOCATABLE ::  LEFTPNS(:,:,:),SUMR(:,:,:)
...
c calcualte perturb t*
      IF (OPT('SOFIELD ')) THEN
      IF (I1.EQ.IMPLAYER) THEN

      ALLOCATE(LEFTPNS(NSPD*LMMAXD,NSPD*LMMAXD,IRMD))
...
c transform left solution back to old mesh
     ...
     +                    RLLTEMP,PNSTEMP,IRMD)
        DO IR=1,IRWS
         DO LM2=1,LMMAXSO
          LEFTPNS(LM1,LM2,IR)=PNSTEMP(IR,LM2)
         ENDDO
        ENDDO
       ENDDO ! LM1
       DEALLOCATE(RLLLEFT)
     ...
     +        (LEFTPNS(LM3,LM1,IR))*PNS_SO(LM3,LM2,IR,1) ! left solution
