C 20.09.95 ***************************************************************
      SUBROUTINE INTIN(G,F,V,E,L,NNE,VALU,SLOP,K1,K2,KC,DG,A,B,Z,NONSRA)
c ************************************************************************
c
C     .. Scalar Arguments ..
      DOUBLE PRECISION A,B,DG,E,SLOP,VALU,Z
      INTEGER K1,K2,KC,L,NNE,NONSRA
C     ..
C     .. Array Arguments ..
      DOUBLE PRECISION F(*),G(*),V(*)
C     ..
C     .. Local Scalars ..
      DOUBLE PRECISION AF1,AF2,AF3,AG1,AG2,AG3,B1,B2,C,DET,DF1,DF2,DF3,
     +                 DG1,DG2,DG3,DR,EA,FF,FLLP1,GG,H83,PHI,Q,R,R1,R2,
...
C     ..
   ...
   20   CONTINUE
      ...
      END IF
c     crit=(g(k)-g(k+1))/(g(k+1)-g(k+2))
c     if(k.gt.k2.and.crit.gt.0.D0) goto 2
   30 KC = K
      DG = DG3
      END
