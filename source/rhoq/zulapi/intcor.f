c 20.09.95 ***************************************************************
      SUBROUTINE INTCOR(F1,F2,RHO,G,F,V,VALUE,SLOPE,L,NN,E,SUM,NRE,VLNC,
     +                  A,B,Z,RN,NR,TOL,IRM,IPR,NITMAX,NONSRA)
c ************************************************************************
C     .. Parameters ..
      include 'inc.p'
c      INTEGER LMAXD
...
C     .. Scalar Arguments ..
      ...
      INTEGER IPR,IRM,L,NITMAX,NN,NONSRA,NR,NRE
      LOGICAL VLNC
C     ..
C     .. Array Arguments ..
...
C     ..
C     .. Local Scalars ..
      DOUBLE COMPLEX ARG,CAPPAI,CXXX,DOFE
      DOUBLE PRECISION C,DE,DG1,DG2,DPSI1,DPSI2,DRDIKC,E1,E2,EA,GKC2,PI,
     +                 PKC1,PKC2,PSI1,PSI2,Q,QKC1,QKC2,RATIO,RATIO1,RE,
     +                 RKC,RPB,SLOP,TSRME,VALU,VME,XXX,ZZ
      INTEGER IR,K,K2,KC,NITER,NNE,NREM1,NREM2
C     ..
C     .. Local Arrays ..
      DOUBLE COMPLEX HL(LMAXD+2)
C     ..
C     .. External Subroutines ..
      EXTERNAL HANKEL,INTIN,INTOUT,RCSTOP
C     ..
C     .. Intrinsic Functions ..
      INTRINSIC ABS,DATAN,EXP,LOG,MAX0,MIN0,REAL,SQRT
C     ..
      PI = 4.D0*DATAN(1.D0)
      ZZ = Z + Z
      C = 274.0720442D0
      IF (NONSRA.EQ.1) C = 1.0D0
      EA = EXP(A)
      NITER = 0
      E1 = F1
      E2 = F2
...
c--->   single site  boundary condition
            VME = -E
            CXXX = (1.D0-VME/C/C)*VME
            IF (NONSRA.EQ.1) CXXX = VME
            CAPPAI = SQRT(-CXXX)
c
c           next line added by p.zahn, 04.09.98
c           prevents a change of sign of SLOP on machines
c           where cappai = -i * sqrt(-e)
            IF (DIMAG(CAPPAI).LT.0.D0) CAPPAI = - CAPPAI
c
            ARG = CAPPAI*RN
...
c---<
        ...
        IF (NN.EQ.0) K2 = NRE/3
        NNE = 0
        CALL INTIN(G,F,V,E,L,NNE,VALU,SLOP,NRE,K2,KC,DG2,A,B,Z,NONSRA)
        ...
        PKC2 = .5D0 - DATAN(RKC*DPSI2/PSI2)/PI

        CALL INTOUT(G,F,V,E,L,NNE,KC,DG1,A,B,Z,NONSRA)

        PSI1 = G(KC)
        DPSI1 = DG1/DRDIKC
        QKC1 = PSI1*PSI1 + DPSI1*DPSI1*RKC*RKC
        PKC1 = .5D0 - DATAN(RKC*DPSI1/PSI1)/PI
        IF (NNE.EQ.9) NNE = 0
        IF (NNE.EQ.NN) THEN
          RATIO1 = GKC2/G(KC)
