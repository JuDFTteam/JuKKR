...
C     .. Intrinsic Functions ..
      INTRINSIC SQRT
C     ..
C     .. For special output of the potential (G. Geranton, 2014) ..
...
c
c           Write out the spher. sym. part of the exchange field
c           (only once, therefore the following if then endif)
c           Changed by G.Geranton, 2014            
            IF (IS.eq.1) THEN 
              IF (TEST('Exfield ')) THEN
                 WRITE (filename, fmt='(a,I2.2,a)')
     &                     'exfield_at',IH,'_LM01'
                 OPEN (20, file=filename)
c                WRITE (20,FMT=9080) (VM2Z(IR,2)-VM2Z(IR,1),IR=1,NR)
                 WRITE (20,FMT=9080) ((VM2Z(IR,NSPIN* (IH-1) + 2) -
     &                     VM2Z(IR,NSPIN* (IH-1) + 1))*0.5*
     &                     DSQRT(16.0D0*DATAN(1.0D0)),IR=1,NR)
                 CLOSE(20)
              ENDIF
            ENDIF
...
c                 Write out the non-spher. sym. part of the exchange field
c                 (only once, therefore the following if then endif)
c                 Changed by G.Geranton, 2014 
                  IF (IS.eq.1) THEN
                   IF (TEST('Exfield ')) THEN
                     WRITE (filename, fmt=9090)'exfield_at',IH,'_LM',LM
                     OPEN (20, file=filename)
c                     WRITE (20,FMT=9080) (0.0D0,IR=1,IRMIN-1)
c                     DO 31 IR = IRMIN,NR
                      DO 31 IR = 1,NR
c                     WRITE (20,FMT=9080) (VINS(IR,LM,2)-VINS(IR,LM,1))
                      WRITE (20,FMT=9080) (VINS(IR,LM,NSPIN*(IH-1) + 2)-
     &                         VINS(IR,LM,NSPIN*(IH-1) + 1))*0.5
   31                CONTINUE
                     CLOSE(20)
                   ENDIF
                  ENDIF
...
c
 ...
 9080 FORMAT (1p,1d20.13)
 9090 FORMAT (a10,I2.2,a3,I2.2)
      END
c ************************************************************************
...
C     .. Save statement ..
      ...
      DATA ISHAPE / 0 /
C     ..
C     .. For special output of the radial mesh (G. Geranton, 2014) ..
      CHARACTER(len=25) :: filename
C     ..
c-----------------------------------------------------------------------
c
...
c           Write out the radial mesh for testing
c           Changed by G.Geranton, 2014     
            IF (TEST('Exfield ')) THEN
               WRITE (filename, fmt='(a,I2.2)')'rmesh_at',IH
               OPEN (20, file=filename)
               WRITE (20,FMT=9015) (R(IR,IH),IR=1,NR)
               CLOSE(20)
            END IF
