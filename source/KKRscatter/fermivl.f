     ...
     &                 PMUP,PMDOWN,SPERTURB)
      implicit none
c ************************************************************************
c calculate fermi velocity, spin expectation value, scattering probability
c solving Boltzmann equation, Dyakonov-Perel mechanism 
c
c required kpoints_FS file
c
c modified by N. H. Long, Juelich, 24.02.11
c-----------------------------------------------------------------------
c     .. parameters ..
      include 'inc.p'
      include 'inc.cls'
...
c find symmetry, calculate BZ volumn
...
c if test option wrtcoeff, writing coefficient after pertubation
     ...
     +  open(unit=30, file="coefficients_so", form='unformatted')
        ...
        S_12=CZERO
c read in file DENS calculated from NORMCOEFF_SO
        READ(24) IWRITE
        DO I1=1,NATYPD
         DO I1SP1=1,NSPOD
          DO I2SP1=1,NSPOD
...
c  for spin-orbit coupling, double size for G_free G_LL'(k) 
        IF (NSPD == 2) THEN
          DO LM1=1,ALM
            DO LM2=1,ALM
...
c construct the matrix M=[1 - G^r*delta t]
        IS1=1
        IS2=1
        TMAT_1=CZERO
        TMAT_2=CZERO
...
c big t-matrix for SOFIELD, nonzero only when I1=IMPLAYER
...
c use lapack routine to calculate the eigenvalues 
...
c finding the zero value and degeneracy
        IENT=0
          MINLM1=1D6
        DO LM1=1,ALMSO
        ...
        ENDDO
          ENT0=2
           DO 101 IENT=2,ENT(LM1TAKE)          
            ...
            ENDIF
101      CONTINUE 
c in case of more than double degeneracy
           IF (ENT0.NE.2) THEN
            CKN(:,5)=CKN(:,2)
            CKN(:,2)=CKN(:,ENT0)
            CKN(:,ENT0)=CKN(:,5)
...
c compare which is belong to a higher energy level
...
c write down coefficients SOFIELD 
       IF (TEST('wrtcoeff')) THEN
        ...
        ENDIF
      RES=CZERO
c rearrange: higher energy first
      IF (-DBLE(PMDOWN(K)/DELTALAMDAE(K))
     +     .GT.-DBLE(PMUP(K)/DELTALAMDAE(K))) THEN
       RES=PMUP(K)
       PMUP(K)=PMDOWN(K)
