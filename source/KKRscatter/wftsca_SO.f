C ************************************************************************
      SUBROUTINE WFTSCA_SO(DRDI,EFAC,LMAX,PZ,QZ,IRMIN,IRWS,IPAN,IRCUT,
     +            FZ,SZ,NSRA,PZLM,QZLM,PZEKDR,QZEKDR,EK,LOFLM,LMMAXD)
C ************************************************************************
c                 R. Zeller      Oct. 1993
c-----------------------------------------------------------------------
C     .. Parameters ..
...
C     .. Array Arguments ..
     ...
     +               PZEKDR(LMMAXD,IRMIN:IRMD,2),
     +               PZLM(LMMAXD,IRMIN:IRMD,2),QZ(IRMD,0:LMAXD),
     +               QZEKDR(LMMAXD,IRMIN:IRMD,2),
     +               QZLM(LMMAXD,IRMIN:IRMD,2),SZ(IRMD,0:LMAXD)
...
c 
c---> the wavefunctions are needed for the two spin-directions; they 
c     are simply copied...
c
c      PZLM(LMMAX+1:2*LMMAX,:,:)=PZLM(1:LMMAX,:,:)
c      QZLM(LMMAX+1:2*LMMAX,:,:)=QZLM(1:LMMAX,:,:)

c      PZEKDR(LMMAX+1:2*LMMAX,:,:)=PZEKDR(1:LMMAX,:,:)
c      QZEKDR(LMMAX+1:2*LMMAX,:,:)=QZEKDR(1:LMMAX,:,:)
