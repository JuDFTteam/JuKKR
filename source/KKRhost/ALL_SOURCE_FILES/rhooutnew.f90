...
subroutine rhooutnew(nsra, lmmaxd, lmmaxso, lmax, gmatll, ek, lmpot, df, &
  npan_tot, ncheb, cleb, icleb, iend, irmdnew, thetasnew, ifunm, imt1, lmsp, &
  rll, rllleft, sllleft, cden, cdenlm, cdenns, rho2nsc, corbital, gflle_part, &
  rpan_intervall, ipan_intervall, ntotd)
  ...
  double precision, dimension (ntotd*(ncheb+1), nfund), &
    intent (in) :: thetasnew
...
! Note that SLL is not needed for calculation of density, only needed for calculation of Green function
  double complex, dimension (nsra*lmmaxso, lmmaxso, irmdnew), &
    intent (in) :: rll
  double complex, dimension (nsra*lmmaxso, lmmaxso, irmdnew), &
    intent (in) :: rllleft
  double complex, dimension (nsra*lmmaxso, lmmaxso, irmdnew), &
...
! For orbital moment
    if (corbital/=0) then
      call zgemm('N', 'N', lmmaxso, lmmaxso, lmmaxso, cone, &
        loperator(1,1,corbital), lmmaxso, wr(1,1,ir), lmmaxso, czero, pnsi, &
        lmmaxso)
      do lm1 = 1, lmmaxso
        do lm2 = 1, lmmaxso
