subroutine regsol(cvlight, e, nsra, dlogdp, fz, hamf, mass, pz, dror, r, s, &
  ...
  ! ..
  ! .. Local Scalars ..
  complex (kind=dp) :: dfd0, dpd0, fip0, fip1, hamf1, k1f, k1p, k2f, k2p, k3f, &
  ...
  do l = 0, lmaxd
    ...
    do ip = 1, ipan
      ...
      do ir = irs + 5, ire
        ...
        pip1 = pip0 + (1901.0e0_dp*dpdi(0)-2774.0e0_dp*dpdi(-1)+2616.0e0_dp* &
          dpdi(-2)-1274.0e0_dp*dpdi(-3)+251.0e0_dp*dpdi(-4))/720.0e0_dp
        fip1 = fip0 + (1901.0e0_dp*dfdi(0)-2774.0e0_dp*dfdi(-1)+2616.0e0_dp* &
          dfdi(-2)-1274.0e0_dp*dfdi(-3)+251.0e0_dp*dfdi(-4))/720.0e0_dp
        ...
        pip0 = pip0 + (251.0e0_dp*dpdi(0)+646.0e0_dp*dpdi(-1)-264.0e0_dp*dpdi( &
          -2)+106.0e0_dp*dpdi(-3)-19.0e0_dp*dpdi(-4))/720.0e0_dp
        fip0 = fip0 + (251.0e0_dp*dfdi(0)+646.0e0_dp*dfdi(-1)-264.0e0_dp*dfdi( &
          -2)+106.0e0_dp*dfdi(-3)-19.0e0_dp*dfdi(-4))/720.0e0_dp
