...
contains
  ...
  subroutine wmatldau(ntldau,itldau,nspin,denmatc,lopt,ueff,jeff,uldau,wldau,eu,edc,&
    ...
    real (kind=dp), dimension (mmaxd, mmaxd, nspind) :: denmat
    complex (kind=dp), dimension (mmaxd, mmaxd, nspind) :: vldau
      ...
      if (lopt(i1)>=0) then
        ...
        do is = 1, nspin
          ...
          if (iprint>1) then
            ...
            call cmatstr(str15, 15, denmatc(1,1,ipot), mmaxd, mmax, 0, 0, 0, 1e-8_dp, 1337)
          end if
          ! -------------------------------------------------------------------
        ...
        do is = 1, nspin
          ...
          ! -------------------------------------------------------------------
          do m1 = 1, mmax
            vldau(m1, m1, is) = vldau(m1, m1, is) - ueff(i1)*(dentot-0.5e0_dp*factor) + jeff(i1)*(dentots(is)-0.5e0_dp*factor)
          end do
        end do
        ! ----------------------------------------------------------------------
        ...
        ! Relativistic case, see the paper H. Ebert et al., Sol. Stat. Comm. 127
        ! (2003) 443
        ! Calculate EDC
        do is = 1, nspin
          ...
          do js = 1, nspin
            do m4 = 1, mmax
              do m3 = 1, mmax
                do m2 = 1, mmax
                  do m1 = 1, mmax
                    eu(i1) = eu(i1) + denmat(m1, m2, is)*uldau(m1, m2, m3, m4, i1)*denmat(m3, m4, js)
                  end do
                end do
              end do
            end do
        ...
        do is = 1, nspin
          do m4 = 1, mmax
            do m3 = 1, mmax
              do m2 = 1, mmax
                do m1 = 1, mmax
                  eu(i1) = eu(i1) - denmat(m1, m2, is)*uldau(m1, m4, m3, m2, i1)*denmat(m3, m4, is)
                end do
              end do
            end do
          end do
        ...
        do is = 1, nspin
          if (iprint>0) then
            write (str15, '(4X,"> ",A,I1)') 'ISPIN = ', is
            call cmatstr(str15, 15, vldau(1,1,is), mmaxd, mmax, 0, 0, 0, 1e-8_dp, 1337)
          end if
          ! -------------------------------------------------------------------
          ...
          do m2 = 1, mmax
            do m1 = 1, mmax
              wldau(m1, m2, is, i1) = real(vldau(m1,m2,is))
            end do
          end do
        end do
        ! ----------------------------------------------------------------------
...
150 format (27x, a, /)
 ...
 !-------------------------------------------------------------------------------
  ...
  subroutine rwrite(z, mmaxd, mmax, ifile)
    ...
    real (kind=dp), dimension (mmaxd, mmaxd), intent (in) :: z
    ! .. Local variables
    integer :: m1, m2
