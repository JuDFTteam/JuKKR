...
! Local parameters:                                                     
  ...
  double precision :: costbn, detop, ddet33, dnrm2, omcos, sintbn, sinpb3, &
    tiny, twopi, vfac
...
! ----- First calculate determinant.
    detop = ddet33(symopm)
    if (dabs(dabs(detop)-1.0d0)>tiny) call errmsg( &
...
! ------- allow for abbreviations as 'D', 'X', 'Y' or 'Z'
      sinpb3 = dsqrt(.75d0)
      if (dabs((sinpb3-dabs(vecg(1)))*(sinpb3-dabs(vecg(2)))*(sinpb3- &
        dabs(vecg(3))))>tiny) then
        do j = 3, 1, -1
          vfac = dabs(vecg(j))
