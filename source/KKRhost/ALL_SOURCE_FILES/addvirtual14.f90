...
! relative to the 1st imp atom, otherwise relative to the lattice coords (absolute coords).
  ...
  j = 0
  do j = 1, 3
    if (abs(ratomimp(j,1))>1d-8) labscord = .true.
  end do
  ...
  do n1 = -nmax, nmax
    do n2 = -nmax, nmax
      do n3 = -nmaxz, nmaxz
        ...
        vec1(1:3) = dble(n1)*bravais(1:3, 1) + dble(n2)*bravais(1:3, 2) + &
          dble(n3)*bravais(1:3, 3)
