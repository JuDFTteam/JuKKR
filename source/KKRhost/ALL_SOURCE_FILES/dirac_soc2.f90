...
contains
  ...
  subroutine dirabmsoc2(getirrsol, c, socscl, it, e, l, mj, kap1, kap2, pis, cg1, cg2, cg4, cg5, cg8, v, b, z, nucleus, r, drdi, dovr, nmesh, dxp, pr, qr, pi, qi, d_p, dq, nrmax)
    ...
    complex (kind=dp) :: d_p(2, 2, nrmax), dq(2, 2, nrmax), dxp(2, 2), pi(2, 2, nrmax), pr(2, 2, nrmax), qi(2, 2, nrmax), qr(2, 2, nrmax)

    ! Local variables
    complex (kind=dp) :: aa11, aa12, aa21, aa22, arg, bb1, bb2, bpp, bqq, cfac, d14, db14, dbh, dh, diffa, diffb, dv14, dvh, emvpp, emvqq, s0, t0
    real (kind=dp) :: acorr(0:nabm-1), acorr0(0:nabm-1), apred(nabm), apred0(nabm), astep, b14, bc(0:npemax), bh, bhlp(nabm+4), cgd(2), cgmd(2), cgo, cgoz, cgz(2), &
      cm(npemax, npemax), cmi(npemax, npemax), csqr, dbdr(nrmax), dbhlp(nabm+4), dhlp(nabm+4), dvdr(nrmax), dvhlp(nabm+4), gam(2), gpm, hlp(nabm+4), hlp1, kap(2), kpx(2), kpy(2), &
      r14, rh, rhlp(nabm+4), rpwgpm, rr, sk(2), sk1, sk2, so2, so6, srk, tz, v14, vc(0:npemax), vh, vhlp(nabm+4), x14, xh
    ...
    do n = nabm + 1, nmesh
      ...
      bpp = b(n)*drdi(n)
      do i = 1, nsol
        socpp(i) = dovr(n)*csqr*(dvdr(n)-dbdr(n)*cgmd(i))/(e-v(n)+csqr+b(n)*cgmd(i))**2
      ...
      do jcorr = 1, itmax
        ...
        do j = 1, nsol
          do i = 1, nsol
            ...
            pold(i, j) = pnew(i, j)
            qold(i, j) = qnew(i, j)
            d_p(i, j, n) = -kpx(i)*pnew(i, j)*dovr(n) + (emvqq+bqq*cgmd(i))*qnew(i, j)
            dq(i, j, n) = kpx(i)*qnew(i, j)*dovr(n) + (emvpp+bpp*cgd(i))*pnew(i, j) + (bpp*cgo+socpp(k)*cgoz)*pnew(k, j) + socpp(i)*cgz(i)*pnew(i, j)
            pnew(i, j) = pr(i, j, n-1)
            qnew(i, j) = qr(i, j, n-1)
            do ic = 0, nacorr
...
100   end do
      if (t_inc%i_write>0) write (1337, 140) kap1, n, r(n), diffa, diffb, it, l, int(2*mj), 'REG'
...
110   continue
    ...
    if (ndiv/=0) then
      ...
      do j = 1, nsol
        do i = 1, nsol
          k = 3 - i
          dq(i, j, n) = kpx(i)*qi(i, j, n)*dovr(n) + (emvpp+bpp*cgd(i))*pi(i, j, n) + (bpp*cgo+socpp(k)*cgoz)*pi(k, j, n) + socpp(i)*cgz(i)*pi(i, j, n)
        end do
      end do
      ...
      do irk = 1, (nabm-1)*ndiv
        ...
        do j = 1, nsol
          do i = 1, nsol
            ...
            mq2(i, j) = kpx(i)*q2(i, j)*dh/rh + (emvpp+bpp*cgd(i))*p2(i, j) + (bpp*cgo+socpp(k)*cgoz)*p2(k, j) + socpp(i)*cgz(i)*p2(i, j)
          end do
        end do
        ...
        do j = 1, nsol
          do i = 1, nsol
            ...
            mq3(i, j) = kpx(i)*q3(i, j)*dh/rh + (emvpp+bpp*cgd(i))*p3(i, j) + (bpp*cgo+socpp(k)*cgoz)*p3(k, j) + socpp(i)*cgz(i)*p3(i, j)
          end do
        end do
        ...
        bpp = b14*d14
        do i = 1, nsol
          socpp(i) = (d14/r14)*csqr*(dv14-db14*cgmd(i))/(e-v14+csqr+b14*cgmd(i))**2
        ...
        do j = 1, nsol
          do i = 1, nsol
            ...
            mq4(i, j) = kpx(i)*q4(i, j)*d14/r14 + (emvpp+bpp*cgd(i))*p4(i, j) + (bpp*cgo+socpp(k)*cgoz)*p4(k, j) + socpp(i)*cgz(i)*p4(i, j)
          end do
        end do
        ...
        do j = 1, nsol
          do i = 1, nsol
            ...
            mq1(i, j) = kpx(i)*q1(i, j)*d14/r14 + (emvpp+bpp*cgd(i))*p1(i, j) + (bpp*cgo+socpp(k)*cgoz)*p1(k, j) + socpp(i)*cgz(i)*p1(i, j)
          end do
        end do
...
120   end do
      if (t_inc%i_write>0) write (1337, 140) kap1, n, r(n), diffa, diffb, it, l, int(2*mj), 'IRR'
...
130   continue
    ...
    call cinit(2*2*nrmax, qi)
140 format (' PRE/CORR NOT CONV. IN <DIRABMSOC2> ', 2i4, f10.7, 2x, 4e12.4, 3i2, '/2 ', a3)
