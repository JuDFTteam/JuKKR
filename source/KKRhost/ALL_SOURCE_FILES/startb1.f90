...
! .. In/Out variables
  ...
  double precision, dimension (irmind:irm, lmpot, nspotd), &
...
!             READ (IFILE,*) Z1,RWS(IH),EFNEW,VBC(ISPIN)
        if (zat(ih)<0.d0) zat(ih) = z1
        if (z1/=zat(ih) .and. zat(ih)>=0.d0) then
...
!-------------------------------------------------------------------
        if (efnew/=0.0d0 .and. i==1) efermi = efnew
!-------------------------------------------------------------------
...
!----------------------------------------------------------------------
        call calrmt(ipf, ipfe, ipe, imt(ih), zat(ih), rmt(ih), rws(ih), &
          rmtnew(ih), alat, drdi(1,ih), a(ih), b(ih), irws1, r(1,ih), io, ins)
