SUBROUTINE dirabmop(getirrsol,c,it,e,l,mj,kap1,kap2,pis,cg1,cg2,  &
    cg4,cg5,cg8,ameopo,v,b,at,z,nucleus,r,drdi,  &
    dovr,nmesh,pr,qr,pi,qi,dp,dq,ap,aq,lop,ntmax, nlamax,nkmmax,nrmax)
!   ********************************************************************
!   *                                                                  *
...
DOUBLE PRECISION AMEOPO(NKMMAX,NKMMAX,NLAMAX,3),AP(2,2,NRMAX), &
              AQ(2,2,NRMAX),AT(NRMAX,NLAMAX,3,NTMAX),B(NRMAX), &
...
! Local variables
DOUBLE COMPLEX AA11,AA12,AA21,AA22,ARG,BB1,BB2,BPP,BQQ,CFAC,CGO, &
           D14,DETD,DH,DIFFA,DIFFB,EMVPP,EMVQQ,MP1(2,2),MP2(2,2), &
  ...
  DO j = 1,nsol
    DO i = 1,nsol
      ...
      dp(i,j,n) = -kap(i)*dovr(n)*pnew(i,j)  &
          + (emvqq+bqq*cgmd(i)+aq(i,j,n))*qnew(i,j)
      dq(i,j,n) = kap(i)*dovr(n)*qnew(i,j)  &
          + (emvpp+bpp*cgd(i)+ap(i,j,n))*pnew(i,j) + (bpp*cgo+ap(k,j,n))*pnew(k,j)
    END DO
  END DO
