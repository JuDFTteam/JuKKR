...
contains
  ...
  subroutine dirabmop(getirrsol, c, it, e, l, mj, kap1, kap2, pis, cg1, cg2, cg4, cg5, cg8, ameopo, v, b, at, z, nucleus, r, drdi, dovr, nmesh, pr, qr, pi, qi, d_p, dq, ap, aq, &
    ...
    logical :: getirrsol
    integer :: it, kap1, kap2, l, lop, nkmmax, nlamax, nmesh, nrmax, ntmax, nucleus, z
    real (kind=dp) :: ameopo(nkmmax, nkmmax, nlamax, 3), ap(2, 2, nrmax), aq(2, 2, nrmax), at(nrmax, nlamax, 3, ntmax), b(nrmax), dovr(nrmax), drdi(nrmax), r(nrmax), v(nrmax)
    complex (kind=dp) :: d_p(2, 2, nrmax), dq(2, 2, nrmax), pi(2, 2, nrmax), pr(2, 2, nrmax), qi(2, 2, nrmax), qr(2, 2, nrmax)

    ! Local variables
    complex (kind=dp) :: aa11, aa12, aa21, aa22, arg, bb1, bb2, bpp, bqq, cfac, cgo, d14, detd, dh, diffa, diffb, emvpp, emvqq, mp1(2, 2), mp2(2, 2), mp3(2, 2), mp4(2, 2), &
      mq1(2, 2), mq2(2, 2), mq3(2, 2), mq4(2, 2), p1(2, 2), p2(2, 2), p3(2, 2), p4(2, 2), pc(2, 2, -npemax:mpsmax), pnew(2, 2), pold(2, 2), q1(2, 2), q2(2, 2), q3(2, 2), q4(2, 2), &
    ...
    if (l==lop) then
      do j = 1, nsol
        ...
        do i = 1, nsol
          ...
          do ms = 1, 2
            do n = 1, nmesh
              ap(i, j, n) = ap(i, j, n) + at(n, 1, ms, it)*ameopo(ikmi, ikmj, 1, ms)
              aq(i, j, n) = aq(i, j, n) + at(n, 1, ms, it)*ameopo(imkmi, imkmj, 1, ms)
            end do
          end do
        end do
      end do
    ...
    if (it==1 .and. l==2 .and. mj>l .and. nmesh<0) then
      do i = 1, nmesh, 30
        if (t_inc%i_write>0) write (1337, '(a,3i4,2e12.5)') 'dirac A:', l, ikm(1), i, ap(1, 1, i), aq(1, 1, i)
      end do
      ...
      do jcorr = 1, itmax
        ...
        do j = 1, nsol
          do i = 1, nsol
            ...
            pold(i, j) = pnew(i, j)
            qold(i, j) = qnew(i, j)
            d_p(i, j, n) = -kap(i)*dovr(n)*pnew(i, j) + (emvqq+bqq*cgmd(i)+aq(i,j,n))*qnew(i, j)
            dq(i, j, n) = kap(i)*dovr(n)*qnew(i, j) + (emvpp+bpp*cgd(i)+ap(i,j,n))*pnew(i, j) + (bpp*cgo+ap(k,j,n))*pnew(k, j)
...
110   continue
    ...
    if (ndiv/=0) then
      ...
      do j = 1, nsol
        do i = 1, nsol
          k = 3 - i
          dq(i, j, n) = kap(i)*qi(i, j, n)*dovr(n) + (emvpp+bpp*cgd(i))*pi(i, j, n) + bpp*cgo*pi(k, j, n)
        end do
      end do
      ...
      do irk = 1, (nabm-1)*ndiv
        ...
        do j = 1, nsol
          do i = 1, nsol
            ...
            mq2(i, j) = kap(i)*q2(i, j)*dh/rh + (emvpp+bpp*cgd(i))*p2(i, j) + bpp*cgo*p2(k, j)
          end do
        end do
        ...
        do j = 1, nsol
          do i = 1, nsol
            ...
            mq3(i, j) = kap(i)*q3(i, j)*dh/rh + (emvpp+bpp*cgd(i))*p3(i, j) + bpp*cgo*p3(k, j)
          end do
        end do
        ...
        do j = 1, nsol
          do i = 1, nsol
            ...
            mq4(i, j) = kap(i)*q4(i, j)*d14/r14 + (emvpp+bpp*cgd(i))*p4(i, j) + bpp*cgo*p4(k, j)
          end do
        end do
        ...
        do j = 1, nsol
          do i = 1, nsol
            ...
            mq1(i, j) = kap(i)*q1(i, j)*d14/r14 + (emvpp+bpp*cgd(i))*p1(i, j) + bpp*cgo*p1(k, j)
          end do
        end do
...
130   continue
    ...
    ! =============================================================== N ====
140 format (' PRE/CORR NOT CONV. IN <DIRAC> ', 2i4, f10.7, 2x, 4e12.4, 3i2, '/2 ', a3)
