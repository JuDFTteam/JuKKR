...
!..
!.. Array Arguments ..
      Complex (Kind=dp) :: fz(irmd, 0:lmaxd), hamf(irmd, 0:lmaxd), mass(irmd), &
        pz(irmd, 0:lmaxd), qz(irmd, 0:lmaxd), sz(irmd, 0:lmaxd)
      Real (Kind=dp) :: dror(irmd), s(0:lmaxd)
      Integer :: ircut(0:ipand)
!..
!.. Local Scalars ..
      Complex (Kind=dp) :: hamf1, k1f, k1p, k2f, k2p, k3f, k3p, k4f, k4p, &
        mass1, qim0, qim1, sim0, sim1
...
!---> start algorithm - 4 point runge kutta with interpolation
          ...
          dror1 = (3.0E0_dp*dror(irs-3)-15.0E0_dp*dror(irs-2)+ &
            45.0E0_dp*dror(irs-1)+15.0E0_dp*dror(irs))/48.0E0_dp
          drsp1 = dror1*sp1
          drsm1 = dror1*sm1
          mass1 = (3.0E0_dp*mass(irs-3)-15.0E0_dp*mass(irs-2)+ &
            45.0E0_dp*mass(irs-1)+15.0E0_dp*mass(irs))/48.0E0_dp
          hamf1 = (3.0E0_dp*hamf(irs-3,l)-15.0E0_dp*hamf(irs-2,l)+ &
            45.0E0_dp*hamf(irs-1,l)+15.0E0_dp*hamf(irs,l))/48.0E0_dp
...
!---> predictor : 5 point adams - bashforth
            ...
            qim1 = qim0 - (1901.0E0_dp*dqdi(0)-2774.0E0_dp*dqdi(1)+2616.0E0_dp &
              *dqdi(2)-1274.0E0_dp*dqdi(3)+251.0E0_dp*dqdi(4))/720.0E0_dp
            sim1 = sim0 - (1901.0E0_dp*dsdi(0)-2774.0E0_dp*dsdi(1)+2616.0E0_dp &
              *dsdi(2)-1274.0E0_dp*dsdi(3)+251.0E0_dp*dsdi(4))/720.0E0_dp
...
!---> corrector : 5 point adams - moulton
            ...
            qim0 = qim0 - (251.0E0_dp*dqdi(0)+646.0E0_dp*dqdi(1)-264.0E0_dp* &
              dqdi(2)+106.0E0_dp*dqdi(3)-19.0E0_dp*dqdi(4))/720.0E0_dp
            sim0 = sim0 - (251.0E0_dp*dsdi(0)+646.0E0_dp*dsdi(1)-264.0E0_dp* &
              dsdi(2)+106.0E0_dp*dsdi(3)-19.0E0_dp*dsdi(4))/720.0E0_dp
...
!---> 2 point corrector - predictor
          ...
          dqdi(0) = mass(ir)*(1.0E0_dp/ek+qim1*fz(ir,l))/pz(ir, l) + &
            drsp1*qim1

          qim0 = qim0 - 0.5E0_dp*dqdi(0) - 0.5E0_dp*dqdi(1)

          dqdi(0) = mass(ir)*(1.0E0_dp/ek+qim0*fz(ir,l))/pz(ir, l) + &
            drsp1*qim0
