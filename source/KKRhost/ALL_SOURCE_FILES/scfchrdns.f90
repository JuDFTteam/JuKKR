...
! Local variables
  double precision :: aux, bdum(3), cff(2, 2), cfg(2, 2), cgf(2, 2), &
    cgg(2, 2), chko(ntmaxchk), chkq(ntmaxchk), chks(ntmaxchk), defermi, dq, &
    mj, mjmax, mjmin, r1m(2, 2), rint(nrmax), totnos
  double precision :: dble, dsqrt
  double complex :: dosl, hffl, jf(nrmax, 2, 2), jg(nrmax, 2, 2), omtl, smtl, &
    wds, wof, wog, wsf, wsg, wt, zf(nrmax, 2, 2), zfjf, zfzf, zg(nrmax, 2, 2), &
    zgjg, zgzg
      ...
      if ((iprint>0) .and. (iecurr==netab)) then
        ...
        if (irel>1) then
          ...
          write (1337, 150) iecurr, eryd, l, it, txtt(it), &
            ...
            omtint(il, it), (hffint(il,it)*1d-3), dosl, smtl, omtl, &
            (hffl*1d-6), ((-jj-1+2*mue), dosm(mue), smtm(mue), omtm(mue), ( &
            hffm(mue)*1d-6), mue=1, jj)
          ...
          write (1337, 240) iecurr, eryd, l, it, txtt(it), &
            'CRYSTAL TERMS       ', dosint(il, it), smtint(il, it), &
            omtint(il, it), (hffint(il,it)*1d-3), dosl, smtl, omtl, &
            (hffl*1d-6), ((-l-1+mm), dosm(mm), smtm(mm), omtm(mm), (hffm(mm)* &
            1d-6), mm=1, jj)
...
! LLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLL
    ...
    if (iprint>0) then
      ...
      write (1337, 160) iecurr, eryd, it, txtt(it), dosi(it), smti(it), &
        omti(it), (hffi(it)*1d-3), dos(it), smt(it), omt(it), (hff(it)*1d-3)
...
! TTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTT
  ...
  if (iecurr==netab) then
    ...
    do it = 1, nt
      ...
      write (1337, 160)(iecurr+1), efermi, 0.0d0, it, txtt(it)
