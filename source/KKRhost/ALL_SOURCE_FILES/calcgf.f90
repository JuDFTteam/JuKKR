...
subroutine calcgf(nk, cgc, gdia, gmdia, goff, gmoff, fdia, fmdia, foff, fmoff, &
  ...
  real (kind=dp) :: cgc(nkmpmax, 2), fdia(nkmmax), fmdia(nkmmax), &
    fmoff(nkmmax), foff(nkmmax), gdia(nkmmax), gmdia(nkmmax), gmoff(nkmmax), &
    goff(nkmmax)
  integer :: kaptab(nmuemax), lbtab(nmuemax), ltab(nmuemax), nmuetab(nmuemax)

  ! Local variables
  integer :: ig, ikm1, ikm2, imkm1, imkm2, j1p05, j2p05, k1, k2, kap1, kap2, &
    l1, l1bar, l2, l2bar, m1, m2, mldn, mlup, mue1m05, mue2m05
