...
! fint\left(r\right)=\int_{0}^{r}f\left(r'\right)dr' \f$
...
subroutine soutk(f, fint, ipan, ircut)
  ...
  do ip = 1, ipan
    ...
    if (ip==1) then
      ...
      ! ----------------------------------------------------------------------
      fint(ist+1) = (f(ist+3)-5.0e0_dp*f(ist+2)+19.0e0_dp*f(ist+1)+9.0e0_dp*f( &
        ist))/24.0e0_dp
    else
      fint(ist) = fint(ist-1)
      ! ----------------------------------------------------------------------
      ! Integrate fint(ist+1) with a 4 point lagrangian
      ! ----------------------------------------------------------------------
      fint(ist+1) = fint(ist-1) + (f(ist+3)-5.0e0_dp*f(ist+2)+19.0e0_dp*f(ist+ &
        1)+9.0e0_dp*f(ist))/24.0e0_dp
    end if
