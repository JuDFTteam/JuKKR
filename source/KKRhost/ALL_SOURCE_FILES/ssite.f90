...
subroutine ssite(iwrregwf, iwrirrwf, nfilcbwf, calcint, getirrsol, soctl, ctl, &
  ...
  real (kind=dp) :: ap(2, 2, nrmax), aq(2, 2, nrmax), c, cff(2, 2), cfg(2, 2), &
    cg1, cg2, cg4, cg5, cg8, cgf(2, 2), cgg(2, 2), ch(2, 2), csqr, ctf(2, 2), &
          ...
          do k2 = 1, nsol
            ...
            if (calcint) then
              ...
              if (ihyper==1) then
                ...
                ! !end of nucleus.eq.0
                call sumupint(bzz(lin,it), cautog, rmehf, ch, 0.0e0_dp, rmehf, &
                  ch, nsol)
              end if
              ! -----------------------------------------------------------------------
        ...
        if (wronski) then
          do i = 1, jtop, 40
            ...
            write (1337, 110, advance='no') it, l, nint(2*mj), i, r(i, im), &
              1.0e0_dp - (zf(i,1,1)*jg(i,1,1)-zg(i,1,1)*jf(i,1,1)+zf(i,2,1)*jg &
              (i,2,1)-zg(i,2,1)*jf(i,2,1))*crsq
            if (nsol==2) then
              write (1337, 120) 1.0e0_dp - (zf(i,1,2)*jg(i,1,2)-zg(i,1,2)*jf(i &
                ,1,2)+zf(i,2,2)*jg(i,2,2)-zg(i,2,2)*jf(i,2,2))*crsq, &
                1.0e0_dp - (zf(i,1,2)*jg(i,1,1)-zg(i,1,2)*jf(i,1,1)+zf(i,2,2)* &
                jg(i,2,1)-zg(i,2,2)*jf(i,2,1))*crsq, &
...
subroutine readwfun(nfil, it, l, mj, nsol, sreg, sirr, ikm1, kap1, ikm2, kap2, &
  ...
  if (sreg=='REG' .or. sreg=='COR') then
    ...
    if (itp/=it .or. lp/=l .or. abs(mjp-mj)>0.001e0_dp .or. nsolp/=nsol .or. &
