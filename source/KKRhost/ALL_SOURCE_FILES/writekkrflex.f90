...
subroutine writekkrflex(natomimp, nspin, ielast, lmpot, alat, natyp, kshape, &
  ...
  if (opt('KKRFLEX ')) then
    ...
    call version_print_header(6699, '; '//md5sum_potential//'; '// &
      md5sum_shapefun)
    write (6699, *) '#', natomimp, nspin, ielast, lmmaxd, korbit
    if (t_tgmat%tmat_to_file) then
      open (69, access='direct', recl=wlength*4*lmmaxd*lmmaxd, file='tmat', &
        form='unformatted')
    end if
    ...
    call version_print_header(91, '; '//md5sum_potential//'; '// &
      md5sum_shapefun)
    ...
    do iatom = 1, natomimp
      ...
      if (noq(i)==0) then
        ...
        if (kshape/=0) then
          write (91, '(5000F25.16)') zat(i1), ((cmom(lm,i1)+cminst(lm,i1))* &
            conc(i1), lm=1, lmpot)
        else
          write (91, '(5000F25.16)') zat(i1), (cmom(lm,i1)*conc(i1), lm=1, &
            lmpot)
        end if
      end if
    end do
