...
contains
  ...
  subroutine operators_for_fscode(korbit, operator_imp)
    ...
    complex (kind=dp), allocatable :: rll(:, :, :, :), sll(:, :, :, :), rllleft(:, :, :, :), sllleft(:, :, :, :), rlltemp(:, :), pnstemp(:, :), pns_so(:, :, :, :), &
      pns_so_all(:, :, :, :, :)
    ...
    if (.not. test('IMP_ONLY')) then ! test option to disable costly recalculation of host operators
        ...
        do ie_num = 1, ie_end
          ...
          if (ie==1) then
            ...
            if (t_wavefunctions%nwfsavemax>0) then ! read wavefunctions?
              ...
              call read_wavefunc(t_wavefunctions, rll, rllleft, sll, sllleft, i1, ie, nsra, lmmaxd, t_inc%irmdnew, 0, 1, rll_was_read_in, sll_was_read_in, rllleft_was_read_in, &
                sllleft_was_read_in)
      ...
      call normcoeff_so(natyp, t_params%ircut, t_params%lmmaxd/(1+korbit), pns_so_all, t_params%thetas, t_params%ntcell, t_params%ifunm, t_params%ipan, t_params%lmsp, t_inc%kvrel, &
        t_params%cleb, t_params%icleb, t_params%iend, t_params%drdi, t_params%irws, 1+korbit, 0)

      if (myrank==master) write (*, *) 'Computing torq operator'
      call normcoeff_so_torq(natyp, t_params%ircut, t_params%lmmaxd/(1+korbit), pns_so_all, t_params%ntcell, t_params%ifunm, t_params%ipan, t_params%lmsp, t_inc%kvrel, &
        t_params%cleb, t_params%icleb, t_params%iend, t_params%drdi, t_params%irws, t_params%visp, t_inc%nspin, t_params%vins, t_params%irmin, 0)
    ...
    if (operator_imp) then
      ...
      call normcoeff_so(natomimp, t_params%ircut, t_params%lmmaxd/2, pns_so_imp, t_params%thetas, t_params%ntcell, t_params%ifunm, t_params%ipan, t_params%lmsp, t_inc%kvrel, &
        t_params%cleb, t_params%icleb, t_params%iend, t_params%drdi, t_params%irws, 1+korbit, 1)

      if (myrank==master) write (*, *) 'Computing impurity torq operator'
      call normcoeff_so_torq(natomimp, t_params%ircut, t_params%lmmaxd/2, pns_so_imp, t_params%ntcell, t_params%ifunm, t_params%ipan, t_params%lmsp, t_inc%kvrel, t_params%cleb, &
        t_params%icleb, t_params%iend, t_params%drdi, t_params%irws, t_imp%vispimp, t_inc%nspin, t_imp%vinsimp, t_params%irmin, 1)
