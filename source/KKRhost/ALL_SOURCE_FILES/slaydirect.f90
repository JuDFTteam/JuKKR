...
! > \f$ SUM_L^{i_1,i_2}=\sum_n
! \frac{Y_L\left(R^{i_1}-R_{n}^{i_2}\right)}{\left|R^{i_1}-R^{i_2}_n
! \right|^{l+1}}\f$

! > @details  \f$i_1\f$, \f$i_2\f$ are plane indeces and the sum is done in
! the
! > \f$i_2\f$ plane
! >
...
! > @note Jonathan Chico Apr. 2018: Removed inc.p dependencies, rewrote to
! Fortran90
! > and renamed the SUM array to SUML to avoid confusion with intrinsic SUM
! function
! -------------------------------------------------------------------------------
