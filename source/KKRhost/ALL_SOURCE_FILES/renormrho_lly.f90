...
subroutine renormrho_lly(cdos_lly, rhospher, irmax, ielast, nspin, ! LLY Lloyd
  ...
  real (kind=dp) :: rhospher(irmax, ielast, nspin, natyp) ! Valence charge
                                                          ! density to be
                                                          ! renormalized in
                                                          ! its spherical
                                                          ! component
  ...
  real (kind=dp) :: denloc(2), denlly(2) ! Spin degeneracy, 2 if nspin=1, 1 if
                                         ! nspin=2
  ...
  do ie = iestart, ieend
    ...
    renorm(1) = (denlly(1)+denlly(2))/(denloc(1)+denloc(2)) ! Integration
                                                            ! weight included
                                                            ! in rhospher
