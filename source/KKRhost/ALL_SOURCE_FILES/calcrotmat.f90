...
!   ********************************************************************
  ...
  double precision :: num, msb05, msb05sq, msb05pw, j, m1, m2, rfac, dom, x, &
    cb05, cb05sq, alfdeg, betdeg, gamdeg, sum, cb05pw
  double precision :: fact(0:100)

  integer :: s, slow, shigh, off, nkmmax, irel, nk, i1, i2, k, l, im1, im2, &
    nmue
    ...
    do im2 = 1, nmue
      ...
      do im1 = 1, nmue
        ...
        if (dabs(betdeg)<1d-8) then
          ...
          dom = (-1.0d0)**(slow-1)*dsqrt(rfac(j+m1)*rfac(j-m1)*rfac(j+m2)*rfac &
            (j-m2))
          sum = 0.0d0
