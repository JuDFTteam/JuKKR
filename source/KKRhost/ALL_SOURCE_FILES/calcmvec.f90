...
subroutine calcmvec(nfilcbwf, splitss, iepath, nepath, irel, iprint, nt, nl, &
  ...
  complex (kind=dp) :: bmvevd(ntmax, 3, nmvecmax), bmvevdl(nlmax, 3, nmvecmax) &
    ...
    w1(nkmmax, nkmmax), w2(nkmmax, nkmmax), w3(nkmmax, nkmmax), zfjf(2, 2), &
    zfzf(2, 2), zgjg(2, 2), zgzg(2, 2)
  logical :: check
  integer :: i, i0, ikm, ikm1, ikm2, ikmcb(2, nkmmax), ikmt1, ikmt2, il, im, &
  ...
  do it = 1, nt
    ...
    do ikm = 1, n
      ...
      if (iprint>3 .and. (t_inc%i_write>0)) write (1337, *) iti, li, mj, nsol, &
        str3, kapcb
    end do
...
100     continue
        call cintabr(zg(1,1,ikmt1), zg(1,1,ikmt2), zgzg, zf(1,1,ikmt1), &
          zf(1,1,ikmt2), zfzf, r2drdi(1,im), nsolcb(ikmt1), nsolcb(ikmt2), &
          jtop, nrmax)

        call cintabr(zg(1,1,ikmt1), jg(1,1,ikmt2), zgjg, zf(1,1,ikmt1), &
        ...
        do k2 = 1, nsolcb(ikmt2)
          ...
          do k1 = 1, nsolcb(ikmt1)
            ...
            do imv = 1, nmvec
              do ipol = 1, npol
                mereg(ikmt1, ikmt2, ipol, imv) = mereg(ikmt1, ikmt2, ipol, &
                  imv) + amemvec(ikm1, ikm2, ipol, imv)*zgzg(k1, k2)
              end do
            end do
        ...
        if (ikmt1==ikmt2) then
          do k2 = 1, nsolcb(ikmt2)
            ...
            do k1 = 1, nsolcb(ikmt1)
              ...
              do imv = 1, nmvec
                do ipol = 1, npol
                  meirr(ikmt1, ikmt2, ipol, imv) = meirr(ikmt1, ikmt2, ipol, &
                    imv) + amemvec(ikm1, ikm2, ipol, imv)*zgjg(k1, k2)
                end do
              end do
            ...
            do imv = 1, nmvec
              do ipol = 1, npol
                meirr(ikmt2, ikmt2, ipol, imv) = meirr(ikmt2, ikmt2, ipol, &
                  imv) - amemvec(imkm2, imkm2, ipol, imv)*zfjf(k2, k2)
              end do
            end do
