...
subroutine invslab(gdi, gup, gdow, gin, icheck)
  ...
  complex (kind=dp) :: gup(ndim_slabinv, ndim_slabinv, nlayerd), gdow(ndim_slabinv, ndim_slabinv, nlayerd), &
    gdi(ndim_slabinv, ndim_slabinv, nlayerd), dmat(ndim_slabinv, ndim_slabinv, nlayerd), &
    dinver(ndim_slabinv, ndim_slabinv, nlayerd), f(ndim_slabinv, ndim_slabinv), e(ndim_slabinv, ndim_slabinv), g(ndim_slabinv, ndim_slabinv), &
    cunit(ndim_slabinv, ndim_slabinv), gin(alm, alm), gdiold(ndim_slabinv, ndim_slabinv, nlayerd)
  ...
  do n = 2, nlayerd
    ...
    call zgemm('N', 'N', ndim_slabinv, ndim_slabinv, ndim_slabinv, cone, dmat(1,1,n-1), ndim_slabinv, &
      gup(1,1,n-1), ndim_slabinv, czero, f(1,1), ndim_slabinv)
    call zcopy(ndim_slabinv*ndim_slabinv, gdi(1,1,n), 1, e(1,1), 1)
    call zcopy(ndim_slabinv*ndim_slabinv, cunit(1,1), 1, dmat(1,1,n), 1)
    call zgemm('N', 'N', ndim_slabinv, ndim_slabinv, ndim_slabinv, -cone, gdow(1,1,n-1), ndim_slabinv, f(1,1), &
  ...
  do n = nlayerd, 1, (-1)
    if (n==nlayerd) then
      ...
      call zgemm('N', 'N', ndim_slabinv, ndim_slabinv, ndim_slabinv, cone, gdow(1,1,n), ndim_slabinv, &
        dmat(1,1,n), ndim_slabinv, czero, f(1,1), ndim_slabinv)
      call bofm(n+1, n+1, e, ndim_slabinv, gin, alm)
      call zgemm('N', 'N', ndim_slabinv, ndim_slabinv, ndim_slabinv, cone, e(1,1), ndim_slabinv, f(1,1), ndim_slabinv, &
        czero, g(1,1), ndim_slabinv)
      call zgemm('N', 'N', ndim_slabinv, ndim_slabinv, ndim_slabinv, cone, gup(1,1,n), ndim_slabinv, g(1,1), &
        ndim_slabinv, czero, f(1,1), ndim_slabinv)
      do lm = 1, ndim_slabinv
        f(lm, lm) = cone + f(lm, lm)
      end do
      call zgemm('N', 'N', ndim_slabinv, ndim_slabinv, ndim_slabinv, cone, dmat(1,1,n), ndim_slabinv, f(1,1), &
    ...
    do irow = (n-1), 1, (-1)
      if (icheck(irow,n)==1) then
        ...
        call zgemm('N', 'N', ndim_slabinv, ndim_slabinv, ndim_slabinv, cone, gup(1,1,irow), ndim_slabinv, &
          e(1,1), ndim_slabinv, czero, f(1,1), ndim_slabinv)
        call zgemm('N', 'N', ndim_slabinv, ndim_slabinv, ndim_slabinv, -cone, dmat(1,1,irow), ndim_slabinv, &
    ...
    do irow = n + 1, nlayerd, 1
      if (icheck(irow,n)==1) then
        ...
        call zgemm('N', 'N', ndim_slabinv, ndim_slabinv, ndim_slabinv, -cone, e(1,1), ndim_slabinv, &
          gdow(1,1,irow-1), ndim_slabinv, czero, f(1,1), ndim_slabinv)
        ! corrected 15.3.2000
        call bofm(irow-1, n, e, ndim_slabinv, gin, alm)
        call zgemm('N', 'N', ndim_slabinv, ndim_slabinv, ndim_slabinv, cone, f(1,1), ndim_slabinv, e(1,1), &
