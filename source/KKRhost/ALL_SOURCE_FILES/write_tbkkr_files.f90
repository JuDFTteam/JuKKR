...
subroutine write_tbkkr_files(lmax, nemb, ncls, natyp, naez, ielast, ins, alat, &
  ...
  write (934, '(I8,4X,A)') lmax, 'lmaxd', lmax, 'lmax', korbit, 'korbit', &
    nspin, 'nspin, used as nspind', & ! write nspin instead of npsind for
                                      ! program to work with nspin==1 case
    nr, 'nrd', nemb, 'nembd', nemb, 'nemb', ncls, &
    'ncls once again, used as nclsd', ncls, 'ncls', & ! ncls instead of nclsd
                                                      ! for smaller files (see
                                                      ! kloopz writeout)
    natyp, 'natypd', natyp, 'natyp', naez, 'naezd', naez, 'naez', naclsmax, &
    'naclsmax, used as naclsd', &  ! naclsmax instead of naclsd for smaller
                                   ! files
