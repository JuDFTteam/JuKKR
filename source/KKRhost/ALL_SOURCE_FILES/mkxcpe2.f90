...
!----------------------------------------------------------------------
  ...
  double precision :: conf, conrs, d, drv1, drv2, drv2s, drv3, drv4, ec, ex, &
...
!     ---begin the spin loop for exchange
    ...
    do jsp = 1, 2
      ...
      if (abs(drv1)<1d-8) then
        ...
        drv2s = 2d0*y*z*ro2(4, jsp) + (z**2-y**2)*ro2(5, jsp) - &
          c/xr*y*(z**2+y**2)
...
! FZ = dF/dzeta [c](A4)
    ...
    eczeta = 4.d0*(zeta**3)*f*(ep-eu+alfm/fzz) + fz*(z4*ep-z4*eu-(1.d0-z4)* &
      alfm/fzz)
