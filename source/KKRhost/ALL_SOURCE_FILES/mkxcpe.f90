    Subroutine mkxcpe(nspin, ir, np, l1max, rv, rholm, vxcp, excp, thet, ylm, &
      dylmt1, dylmt2, dylmf1, dylmf2, dylmtf, drrl, ddrrl, drrul, ddrrul, &
      irmd, lmpotd)
...
!.. Array Arguments ..
      Real (Kind=dp) :: ddrrl(irmd, lmpotd), ddrrul(irmd, lmpotd), &
        ...
        dylmtf(ijd, lmpotd), excp(ijd), rholm(lmpotd, 2), thet(ijd), &
        vxcp(ijd, 2), ylm(ijd, lmpotd)
!..
!.. Local Scalars ..
      Real (Kind=dp) :: agr, agrd, agru, cedg, cedl, chg, cosx, dagrf, dagrfd, &
        dagrfu, dagrr, dagrrd, dagrru, dagrt, dagrtd, dagrtu, ddrr, ddrrd, &
        ddrru, df1, df2, drr, drrd, drru, dt1, dt2, dtf, dzdfs, dzdr, dzdtr, &
        etot0, etota0, g2r, g2rd, g2ru, gggr, gggrd, gggru, grf, grfd, grfu, &
        grgrd, grgru, grr, grrd, grru, grt, grtd, grtu, gzgr, rdspr, ro, rod, &
        rou, rv2, rv3, rvsin1, rvsin2, ry2, rylm, sint1, sint2, smag, spi, &
        tant1, vcg1, vcg2, vcl1, vcl2, vtot1, vtot2, vtota1, vtota2, vxg1, &
        vxg2, vxl1, vxl2, xedg, xedl, zta
      Integer :: idspr, im, ip, l1, ll, llmax, lm, lmax, nn, nn1
!..
!.. Local Arrays ..
      Real (Kind=dp) :: ddry(ijd), ddryd(ijd), ddryu(ijd), drdf(ijd), &
...
! set values later
        Else
          ...
          dzdtr = ((rdt1u(ip)-rdt1d(ip))*ry(ip)-(ryu(ip)-ryd(ip))*rdt1(ip))/ &
            ry2/rv

          dzdfs = ((rdf1u(ip)-rdf1d(ip))*ry(ip)-(ryu(ip)-ryd(ip))*rdf1(ip))/ &
            ry2/rvsin1

          g2r = ddry(ip) + 2.E0_dp*dry(ip)/rv + (rdt2(ip)+rdt1(ip)/tant1+rdf2( &
            ip)/sint2)/rv2
...
! for debug
          Call gxcpt(idspr, chg, zta, agr, agru, agrd, g2r, g2ru, g2rd, gggr, &
            ...
            excp(ip), vxl1, vxl2, vcl1, vcl2, xedl, cedl, vxg1, vxg2, vcg1, &
            vcg2, xedg, cedg)
