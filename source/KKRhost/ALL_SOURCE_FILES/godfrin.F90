...
! Storage for the non-zero elements of matrix A
  double complex, allocatable :: aii(:,:,:), aij(:,:,:), aji(:,:,:)
! Storage for coefficients of recursion
  double complex, allocatable :: x(:,:,:), y(:,:,:), tmp(:,:), c(:,:), d(:,:)
! Extra storage for periodic case
  double complex, allocatable :: borders(:,:,:,:), delta(:,:)
! ----------------------------------------------------------------------
...
! Typical dimensions:
! --> na = nlayers*nlms, nlayers number of layers, nlms is nspin*(lmax+1)^2
...
!
! ************************************************************************************
! WARNING: if the matrix a is not diagonally dominant this routine might be inaccurate
! ************************************************************************************
...
! Block tridiagonal matrix a to be inverted
  double complex, intent(inout) :: a(na,na)
...
! Variables needed to run the PARDISO solver
  ...
  double complex, allocatable :: asparse(:), b(:,:)
! Timing
...
! The non-zero blocks stored in compressed sparse row format
  double complex, intent(out)   :: asparse(ntot)
! Unit matrix
  double complex, intent(out)   :: b(k,k)
...
!  write(*,*) "Debug: ib, i, jb, j, ia, ja, itot"
  do ib=1,nb
    i0 = sum(bdims(1:ib-1)); n = bdims(ib)
    do i=1,n
      ia(i0+i) = itot + 1
!     ------------------------------------------------------------------
!     Columns
!     A(N,1)
      if (periodic .and. ib == nb) then
        jb = 1
        j0 = sum(bdims(1:jb-1)); m = bdims(jb)
        do j=1,m
