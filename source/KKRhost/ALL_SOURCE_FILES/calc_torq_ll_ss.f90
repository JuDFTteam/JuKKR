...
! non-sph. part of the potential
  double complex :: rll(irmd, lmmaxd, lmmaxd), & 
    dens
  double precision :: cleb(*), drdi(irmd), & 
    visp(irmd, *) & ! local variables
  ...
  do lm1p = 1, lmmax
    do ir = 1, irmd
      rsp(ir) = rsp(ir) + rll(ir, lm1p, lm1p)*c0ll*(-1)*(visp(ir,nspin*(iatom- &
        1)+2)-visp(ir,nspin*(iatom-1)+1))*0.5*dsqrt(16.0d0*datan(1.0d0))
! always >= 2 here
    end do
  end do
        ...
        if (lm1p==lm2p) then
          ...
          do ir = irmin, ircut(ipan)
            rges(ir) = rges(ir) + rll(ir, lm2p, lm1p)*cleb(j)*(-1)*(vins(ir, &
              lm3p,nspin*(iatom-1)+2)-vins(ir,lm3p,nspin*(iatom-1)+1))*0.5
          end do
        else
