...
! Compute and write out corrected dos at new (middle) energy points:
    do ie = 2, iemax - 1
      ...
      do ll = 0, lmax + 1
        dosnew(ll) = dos(ll, ie) + 0.5d0*(dos(ll,ie-1)-dos(ll,ie+1))*dcmplx( &
          0.d0, eim)/deltae
      end do

      write (50, 110) enew, dimag(dosnew(lmax+1)), (dimag(dosnew(ll)), ll=0, &
        lmax)
      write (51, 110)(enew-ef)*ev, dimag(dosnew(lmax+1))/ev, &
