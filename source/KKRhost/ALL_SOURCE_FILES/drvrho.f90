...
contains
  ...
  subroutine drvrho_qdos(ldorhoef, rho2ns, r2nef, den, dmuorb, rhotborb, iecurr, eryd, we, ielast, gmatll, vt, bt, r, drdi, r2drdi, zat_in, jws_in, ishift, solver, soctl, ctl, &
    ...
    ! Local variables
    real (kind=dp) :: ameopo(nkmmax, nkmmax, nlamax, 3), at(nrmax, nlamax, 3, ntmax), bcor(ntmax), bcors(ntmax), conc(ntmax), dos(ntmax), dosi(ntmax), efermi, hff(ntmax), &
      hffi(ntmax), mueorb, muespn, nvaltot, omt(ntmax), omti(ntmax), qel(ntmax), rhoorb(nrmax, ntmax), rhochr(nrmax, ntmax), rhospn(nrmax, ntmax)
    real (kind=dp) :: shftef, smt(ntmax), smti(ntmax), pi, sqpi, totdos
    complex (kind=dp) :: bzj(linmax, ntmax), bzz(linmax, ntmax), dosint(nlmax, ntmax), dosl0(nlmax, ntmax), dosm(nmuemax), dzj(linmax, ntmax), dzz(linmax, ntmax), eband, &
      ebandt(ntmax), hffint(nlmax, ntmax), hffl0(nlmax, ntmax), hffm(nmuemax), msst(nkmmax, nkmmax, ntmax), omtint(nlmax, ntmax), omtl0(nlmax, ntmax), omtm(nmuemax), &
      ozj(linmax, ntmax), ozz(linmax, ntmax), p, qzj(linmax, ntmax), qzz(linmax, ntmax), smtint(nlmax, ntmax), smtl0(nlmax, ntmax), smtm(nmuemax), szj(linmax, ntmax), &
      szz(linmax, ntmax)
    complex (kind=dp) :: taut(nkmmax, nkmmax, ntmax), omtls0(nlmax, ntmax, 2)
    complex (kind=dp) :: ozzs(linmax, ntmax, 2), ozjs(linmax, ntmax, 2)
    complex (kind=dp) :: tautlin(linmax, ntmax), tsst(nkmmax, nkmmax, ntmax), tsstlin(linmax, ntmax), tzj(linmax, ntmax), tzz(linmax, ntmax)
    ...
    integer :: i, iecurr, ihyper, ikm1lin(linmax), ikm2lin(linmax), il, imt(ntmax), imue, ip, iprint, iq, iqat(nqmax, ntmax), irel, it, iwrirrwf, iwrregwf, j, lin, lopt(ntmax), &
      mmax, nat(ntmax), netab, nkm, nkmq(nqmax), nl, nlinq(nqmax), nlq(nqmax), nt, nucleus
    integer :: nfilcbwf, iol
    integer :: nsollm(nlmax, nmuemax), ltab(nmuemax), lbtab(nmuemax), kaptab(nmuemax), nmuetab(nmuemax)
    ...
    ! .. dummy arrays
    complex (kind=dp) :: mezj(nkmmax, nkmmax, ntmax, nmvecmax), mezz(nkmmax, nkmmax, ntmax, nmvecmax)
    ...
    if (icall==1) then
      ...
      if (nmvecmax>nmvecmaxd) then
        ...
        write (6, *) ' Increase NVECMAXD in < DRVRHO > ', 'or reduce NMVECMAX in < main1c > '
        stop ' In < DRVRHO > '
      end if
      ...
      open (nfilcbwf, status='SCRATCH', form='UNFORMATTED', access='DIRECT', recl=iol)
    ...
    call ssite(iwrregwf, iwrirrwf, nfilcbwf, calcint, getirrsol, soctl, ctl, eryd, p, ihyper, iprint, ikm1lin, ikm2lin, nlq, nkmq, nlinq, nt, nkm, iqat, tsst, msst, tsstlin, dzz, &
      dzj, szz, szj, ozz, ozj, bzz, bzj, qzz, qzj, tzz, tzj, vt, bt, at, zat, nucleus, r, drdi, r2drdi, jws, imt, ameopo, lopt, solver, cgc, ozzs, ozjs, nlmax, nqmax, linmax, &
    ...
    do ipoint = 1, nqdos           ! qdos ruess
      ...
      call zgemm('N', 'N', lmmaxd, lmmaxd, lmmaxd, cone, gmatll(1,1,iecurr), lmmaxd, tsst(1,1,it), nkmmax, czero, w1, lmmaxd)

      ! ---> taut = t * G * t + t = t * w1 + taut

      call zgemm('N', 'N', lmmaxd, lmmaxd, lmmaxd, cone, tsst(1,1,it), nkmmax, w1, lmmaxd, cone, taut(1,1,it), nkmmax)
      ...
      call scfchrdns(nfilcbwf, r2drdi, jws, imt, shftef, totdos, muespn, mueorb, irel, iprint, nt, nl, nkm, eryd, we, efermi, iec, netab, dos, smt, omt, hff, dosi, smti, omti, &
      ...
      if (itermvdir) then
        ...
        call calcmvec(nfilcbwf, splitss, iepath, nepath, irel, iprint, nt, nl, mezz, mezj, taut, tsst, iqat, nkmq, nkm, iec, netab, igrid(iepath), we, mvevdl0, mvevil1, bmvevdl0, &
          bmvevil1, r2drdi, jws, imt, amemvec, ikmllim1, ikmllim2, imkmtab, ntmax, nlmax, nmuemax, nqmax, nkmmax, nmmax, nmvecmax, nrmax)
    ...
    call scfchrdns(nfilcbwf, r2drdi, jws, imt, shftef, totdos, muespn, mueorb, irel, iprint, nt, nl, nkm, eryd, cone, efermi, iec, netab, dos, smt, omt, hff, dosi, smti, omti, &
      hffi, dosm, dosl0, dosint, smtm, smtl0, smtint, omtm, omtl0, omtint, hffm, hffl0, hffint, bcor, bcors, dzz, dzj, szz, szj, ozz, ozj, bzz, bzj, ozzs, ozjs, omtls0, tautlin, &
      nvaltot, txtt, conc, nat, rhochr, rhospn, rhoorb, qel, gdia, gmdia, goff, ntmax, nlmax, nmuemax, linmax, nrmax, nmmax, nkmmax, eband, ebandt)
