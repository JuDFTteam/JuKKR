...
subroutine generalpot(ifile, natps, natyp, nspin, z, alat, rmt, rmtnew, rws, &
  ...
  efermi, vbc, ecore, lcore, ncore, lmpotd, irmd, irmind)
! **************************************************
...
!.. Array Arguments ..
  double precision :: a(*), b(*), drdi(irmd, *), ecore(20, *), efermi, &
    r(irmd, *), rmt(*), rmtnew(*), rws(*), vbc(2), vins(irmind:irmd, lmpotd, * &
    ), vm2z(irmd, *), z(*)
...
!.. Local Arrays ..
  ...
  double precision :: vm2zb(irmd), vm2z_u(irmd), vins_u(irmind:irmd, lmpotd), &
...
! Cubic spline polynomial is now evaluated.
  ...
  y = a*ya(klo) + b*ya(khi) + ((a**3-a)*y2a(klo)+(b**3-b)*y2a(khi))*(h**2)/ &
    6.d0
  yderiv = (ya(khi)-ya(klo))/h - ((3.d0*a*a-1.d0)*y2a(klo)-(3.d0*b*b-1.d0)*y2a &
    (khi))*h/6.d0
