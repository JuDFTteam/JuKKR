...
contains
  ...
  subroutine ewald2d(lpot, alat, vec1, vec2, iq1, iq2, rm2, nrmax, nshlr, nsr, gn2, ngmax, nshlg, nsg, sum2d, vol, lassld, lmxspd)
    ...
    real (kind=dp) :: dfac(0:2*lassld+1), g(0:lassld), gal(0:lassld), gi(0:4), gr(0:4), pref0(0:lassld), signrzl(0:lassld), ylm(lmxspd), ylmpref(0:lassld), &
      ylmpref1(0:lassld, 0:lassld)
    complex (kind=dp) :: cim(0:lassld), exponl(0:lassld), pref2(lassld), s0(lmxspd), stest(lmxspd), stestnew(lmxspd)
    ! ..
    ! .. Data statements
    data icall/0/
    ! ..
    ...
    if (abs(dq3)<1e-6_dp) then
      ...
      stestnew(1) = stestnew(1) - 2e0_dp*lamda/sqrt(pi) - 2e0_dp*sqrt(pi)/lamda/vol
    ...
    if (abs(dq3)<1e-6_dp) then
      ...
      do l = 0, lmax
        ...
        cim(l) = (-ci)**l
        do m = 1, l
          ylmpref1(l, m) = con1*sqrt(real(2*l+1,kind=dp)/2e0_dp/fpi/dfac(l+m)/dfac(l-m))
        end do
      end do
      ...
      if (s>bound .and. ngmax1==ngmax) write (1337, fmt=120) abs(s), bound, iq1, iq2
    end if
    ! **********************************************************************
...
100 format (5x, 'WARNING 1 : Convergence of 2D-sum is ', 1p, e9.2, ' > ', e9.2, 'LAYER PAIR', 2i6, /, 15x, 'You should use more lattice vectors (RMAX)')
110 format (5x, 'WARNING 2 : Convergence of 2D-sum is ', 1p, e9.2, ' > ', e9.2, 'LAYER PAIR', 2i6, /, 15x, 'You should use more lattice vectors (GMAX)')
120 format (5x, 'WARNING 3 : Convergence of 2D-sum is ', 1p, e9.2, ' > ', e9.2, 'LAYER PAIR', 2i6, /, 15x, 'You should use more lattice vectors (GMAX)')
