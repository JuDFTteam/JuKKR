...
subroutine coredir(it, c, e, l, mj, way, vv, bb, rc, drdic, dovrc, nmatch, &
  ...
  save :: a11, a12, a21, a22, aa11, aa12, aa21, aa22, alpha, bb1, bb2, bc, &
    beta, bova, bpp, bqq, cg1, cg2, cg4, cg5, cg8, csqr, det, diffa, diffb, &
    dmue, dvc, emvpp, emvqq, gam, gpm, h24, i, iv, j, jcorr, k, kap, kap1, &
    kap2, m, mps, n, nn, nsol, pc, pnew, pold, qc, qnew, qold, rpwgpm, rr, tz, &
  ...
  if (way=='INW') then
    ...
    do nn = 1, (nzero-3-nmatch)
      ...
      do j = 1, nsol
        do i = 1, nsol
          pnew(i, j) = wp(i, j, n+1) - h24*(55.0d0*d_p(i,j,n+1)-59.0d0*d_p(i,j &
            ,n+2)+37.0d0*d_p(i,j,n+3)-9.0d0*d_p(i,j,n+4))
      ...
      do jcorr = 1, itmax
        ...
        do j = 1, nsol
          do i = 1, nsol
            ...
            pnew(i, j) = wp(i, j, n+1) - h24*(9.0d0*d_p(i,j,n)+19.0d0*d_p(i,j, &
              n+1)-5.0d0*d_p(i,j,n+2)+d_p(i,j,n+3))
...
100   end do
      if (t_inc%i_write>0) write (1337, 140) kap1, n, rc(n), diffa, diffb, it, &
        l, int(2*mj), ' IN'
...
140 format (' P/C NOT CONV. IN <DIRAC> ', 2i4, 2x, f10.7, 2x, 2e12.4, 3i2, &
    '/2 ', a3)
150 continue
