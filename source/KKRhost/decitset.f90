...
contains
  ...
  subroutine decitset(alat, bravsys, ez, ielast, nlbasis, nrbasis, fileleft, fileright, ins, kvrel, krel, nspin, kmrot, vref, rmtref, nref, refpot, lefttinv, righttinv, vacflag, &
    ...
    complex (kind=dp) :: lefttinv(lmmaxd, lmmaxd, nembd1, nspind, iemxd), righttinv(lmmaxd, lmmaxd, nembd1, nspind, iemxd)
    logical :: vacflag(2)
    ! ..
    ! .. Local scalars ..
    ...
    real (kind=dp) :: bravais(3, 3, 2), rbasis(3, nembd1), qmtet(nembd1), qmphi(nembd1)
    ...
    do ihost = 1, 2
      ...
      call decipothead(ihost, filehost, ilhost, nqhost, vacflag, alat, bravsys, nq, nt, bravais(1,1,ihost), efermi, insh(ihost), krelh(ihost), nspinh(ihost), ins, krel, nspin, &
        kmrot)
    ...
    do ihost = 1, 2
      ...
      if (filehost(1:7)=='vacuum') then
        ...
        write (6, 130) krelh(ihost), nspinh(ihost), insh(ihost), kmrot, nqhost, alat, efermi
        write (6, 140)((bravais(ll,mm,ihost),mm=1,3), ll=1, 3)
        call decipotbas(ihost, iqoff, itoff, nqhost, nthost, rbasis, qmtet, qmphi, noq, kaoez, zat, iqat, conc, irws, ipan, ircut, rr, drdi, visp, nspinh(ihost), krelh(ihost), &
    ...
    do ie = 1, ielast
      ...
      if (krel==0) then
        do i1 = 1, nref
          call calctref13(eryd, vref(i1), rmtref(i1), lmaxd, ih, trefll(1,1,i1), dtrefll(1,1,i1), alpharef(0,i1), dalpharef(0,i1), lmaxd+1, lmgf0d)
        end do
      end if
      ! &&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
        ...
        do ih = 1, nqhost
          ...
          do ispin = 1, nspinh(ihost)
            ...
            do ioq = 1, noq(iqh)
              ...
              call decitmat(eryd, zat(i1), ipan(i1), rr(1,i1), dror(1,i1), visp(1,ipot), ircut(0,i1), rirc, krel, nsra, ins, tmatll, loflm, idoldau, lopt, wldauav, solver, &
                socscl(1,krel*i1+(1-krel)), cscl(1,krel*i1+(1-krel)), zrel(i1), vtrel(1,i1), btrel(1,i1), drdi(1,i1), r2drdirel(1,i1), ipand, irmd, lmaxd, lmaxd+1, lm2d, lmmaxd)
            ...
            if (print_tmat) then
              ...
              call cmatstr(' ', 1, dhmat(1,1,ispin), lmmaxd, lmmaxd, 2*krel+1, 2*krel+1, 0, 1e-8_dp, 6)
              write (1337, *)
            end if
            ! tttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttt
            ...
            call zgetri(lmmaxd, dhmat(1,1,ispin), lmmaxd, ipvt, tmatll, lmmaxd*lmmaxd, info)
          end do
          ! ======================================================================
...
110 format (', ', a)
120 format (a, 2f10.6)
130 format (10x, 'KREL= ', i1, ' NSPIN= ', i1, ' INS= ', i1, ' KMROT= ', i1, /, 10x, 'NAEZ=', i3, ' ALAT= ', f9.6, ' EFERMI= ', f9.6)
140 format (10x, 'BRAVAIS ', /, 10x, 3f8.4, /, 10x, 3f8.4, /, 10x, 3f8.4, /, 10x, 'RBASIS')
