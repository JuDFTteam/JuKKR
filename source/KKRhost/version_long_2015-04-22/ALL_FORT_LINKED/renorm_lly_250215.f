...
! Now the locally-summed charge/energy is in cdos_loc, charge/energy/atom in chadd
          IF (.NOT.OPT('NEWSOSOL')) THEN
           DO ISPIN=1,NSPIN
            ...
            CREN(IE,ISPIN) = CDOS_LLY(IE,ISPIN) / CDOS_LOC(IE,ISPIN)

            ! Apply to DOS of each atom:
            DO I1=1,NATYPD
            CHARGE_LLY(I1,ISPIN) = CHARGE_LLY(I1,ISPIN) +  
     &               DIMAG( CHADD(IE,I1,ISPIN) * CREN(IE,ISPIN) )
            ENDDO
            ENDDO ! ISPIN = 1,NSPIN
           ENDDO ! IE = IESTART,IEEND
          ELSE 
           DO IE=IESTART,IEEND
            ! Renormalization factor per energy:
            CREN(IE,1) = CDOS_LLY(IE,1)/(CDOS_LOC(IE,1)+CDOS_LOC(IE,2))
            ! Apply to DOS of each atom:
            DO ISPIN=1,NSPIN
             DO I1=1,NATYPD
              CHARGE_LLY(I1,ISPIN) = CHARGE_LLY(I1,ISPIN) +  
     &               DIMAG( CHADD(IE,I1,ISPIN) * CREN(IE,1) )
             ENDDO
            ENDDO
           ENDDO ! IE = IESTART,IEEND
          ENDIF 

         IF (NSPIN.EQ.1.OR.OPT('NEWSOSOL')) CREN(:,2) = CREN(:,1) ! If spins are coupled, then only charge density 
...
! Now apply renormalization to energy-integrated charge density & spin density
     ...
     &                         ( CHARGE(I1,1) + CHARGE(I1,2) ) 
         IF (NSPIN.EQ.2) THEN
          IF (ABS(CHARGE(I1,2)-CHARGE(I1,1)).LE.1D-02) THEN
             RENORM_AT(I1,2)=1D0
          ELSE
             RENORM_AT(I1,2) = ( CHARGE_LLY(I1,2) - CHARGE_LLY(I1,1) ) /  ! Spin renormalization
     &                             ( CHARGE(I1,2) - CHARGE(I1,1) ) 
          ENDIF
         ENDIF
c         IF (NSPIN.EQ.1) RENORM_AT(I1,2) = RENORM_AT(I1,2)
