...
c      IF (LLY.GT.0) THEN                                                 ! LLY Lloyd
c
c      DO I1 = 1,NATYP
c       ICELL = NTCELL(I1)
c       IPOT = (I1-1) * NSPIN + 1
c       
c        CALL RHOVALNEW(LDORHOEF,IELAST,NSRA,NSPIN,LMAX,EZ,WEZ,
c     &           ZAT(I1),SOCSCALE(I1),CLEB(1,1),ICLEB,IEND,
c     &           IFUNM1(1,ICELL),LMSP1(1,ICELL),
c     &           NCHEB,NPAN_TOT(I1),NPAN_LOG(I1),
c     &           NPAN_EQ(I1),RMESH(1,I1),IRWS(I1),RPAN_INTERVALL(0,I1),
c     &           IPAN_INTERVALL(0,I1),RNEW(1,I1),VINSNEW,
c     &           THETASNEW(1,1,ICELL),THETA(I1),PHI(I1),I1,IPOT,
c     &           DEN1(0,1,1),ESPV1(0,1),RHO2M1,RHO2M2,MUORB(0,1,I1))
c        DO L = 0,LMAXD1
c         DO IE=1,IELAST
c          DEN(L,IE,IPOT)=DEN1(L,IE,1)
c          DEN(L,IE,IPOT+NSPIN-1)=DEN1(L,IE,NSPIN)
c         ENDDO
c        ENDDO
c        DO ISPIN=1,NSPIN
c         DO LM = 1,LMPOTD
c          DO IR = 1,IRMD
c           RHO2NS(IR,LM,I1,ISPIN) = RHO2M1(IR,LM,ISPIN)
c           R2NEF(IR,LM,I1,ISPIN)  = RHO2M2(IR,LM,ISPIN)
c          END DO
c         END DO
c        END DO
c      ENDDO ! I1
c
c! In case of Lloyds formula renormalize valence charge                   ! LLY Lloyd
c         LMAXP1 = LMAX                                                   ! LLY Lloyd
c         IF (INS.NE.0) LMAXP1 = LMAX + 1                                 ! LLY Lloyd
c         CALL RENORM_LLY(
c     >     CDOS_LLY,IELAST,NSPIN,NATYP,DEN,LMAXP1,CONC,
c     >     1,IELAST,WEZ,IRCUT,IPAN,
c     >     RHO2NS,CDOS1,CDOS2,WEZNORM)
c
c
c      ENDIF                                                              ! LLY Lloyd
