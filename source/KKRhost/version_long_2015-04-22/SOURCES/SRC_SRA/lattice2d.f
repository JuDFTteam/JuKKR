C*==lattice2d.f    processed by SPAG 6.05Rc at 20:22 on 18 May 2004
...
C *  actually needed in EWALD2D                  v.popescu May 2004    *
C *                                                                    *
C **********************************************************************
      IMPLICIT NONE
C     ..
...
C     .. Array arguments ..
      ...
      DOUBLE PRECISION GN(2,NMAXD),RM(2,NMAXD)
      INTEGER NSG(ISHLD),NSR(ISHLD)
C     ..
C     .. Local scalars ..
      DOUBLE PRECISION A,ABSGM,ABSRM,AG,AR,B,DA,DB,GMAX,GX,GY,PI,
     &                 RMAX,RX,RY,VMIN
...
C     .. Local arrays ..
      ...
      DOUBLE PRECISION GNR(NMAXD),RMR(NMAXD),LENGTH(NMAXD)
C     ..
...
C ----------------------------------------------------------------------
            IF ( AR.LE.RMAX ) THEN
               ...
               IF ( NR.GT.NMAXD ) THEN
                  WRITE (6,*) 
     &           'lattice2d: ERROR: Dimension NMAXD in inc.p too small',
     &                 NR,NMAXD
                  STOP 'lattice2d'
               END IF
               CJ(1,NR) = RX
