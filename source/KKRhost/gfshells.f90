...
inner:    do j = 1, natomimp
    ...
    do ns = 1, nshell(0)
      ...
      if (ns<=nb) then
        ...
        write (1337, 170) ns, nsh1(ns), nsh2(ns), (ratom(ii,ns), ii=1, 3), sqrt(ratom(1,ns)**2+ratom(2,ns)**2+ratom(3,ns)**2), str9
      else
        write (1337, 180, advance='no') ns, nsh1(ns), nsh2(ns), (ratom(ii,ns), ii=1, 3), sqrt(ratom(1,ns)**2+ratom(2,ns)**2+ratom(3,ns)**2)
        io = min(2, nshell(ns))
        do i = 1, io
...
110 format (6x, 'Dimension ERROR: please increase the global parameter', /, 6x, a, ' to a value >=', i5, /)
120 format (6x, 'ERROR: there are some inconsistencies in your input', /, 13x, 'not all atoms defined by NATYP have been found', /)
130 format (8x, 'Different shells for GF calculation : ', i3, /)
140 format (8x, 'Reading in cluster impurity sites from file', /, 12x, 'file name        : ', a, /, 12x, 'atoms in cluster : ', i3)
150 format (8x, 'Preparing indexing for impurity GF', /, 11x, '- unsymmetrised GF is written out (v. 20.09.2001)', /, 11x, '- files that will be created: impurity.coefs', /, 41x, &
      'intercell_ref', /, 41x, 'green', /)
160 format (6x, 72('-'), /, 6x, 'shell|', ' IQ ', ' JQ', ' | ', 10x, 'vec R_IJ ', 11x, 'R_IJ   | equiv. pairs', /, 6x, 72('-'))
