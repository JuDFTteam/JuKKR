C*==dirabmop.f    processed by SPAG 6.05Rc at 21:10 on 29 Apr 2002
      SUBROUTINE DIRABMOP(GETIRRSOL,C,IT,E,L,MJ,KAP1,KAP2,PIS,CG1,CG2,
     &                    CG4,CG5,CG8,AMEOPO,V,B,AT,Z,NUCLEUS,R,DRDI,
     &                    DOVR,NMESH,PR,QR,PI,QI,DP,DQ,AP,AQ,LOP,NTMAX,
     &                    NLAMAX,NKMMAX,NRMAX)
C   ********************************************************************
C   *                                                                  *
...
C
     ...
     &        NUCLEUS,Z
      REAL*8 AMEOPO(NKMMAX,NKMMAX,NLAMAX,3),AP(2,2,NRMAX),AQ(2,2,NRMAX),
     &       AT(NRMAX,NLAMAX,3,NTMAX),B(NRMAX),DOVR(NRMAX),DRDI(NRMAX),
     &       R(NRMAX),V(NRMAX)
      ...
      IF ( L.EQ.LOP ) THEN
         DO J = 1,NSOL
            ...
            DO I = 1,NSOL
               ...
               DO MS = 1,2
                  DO N = 1,NMESH
                     AP(I,J,N) = AP(I,J,N) + AT(N,1,MS,IT)
     &                           *AMEOPO(IKMI,IKMJ,1,MS)
                     AQ(I,J,N) = AQ(I,J,N) + AT(N,1,MS,IT)
     &                           *AMEOPO(IMKMI,IMKMJ,1,MS)
                  END DO
               END DO
            END DO
         END DO
...
C
      ...
      IF ( IT.EQ.1 .AND. L.EQ.2 .AND. MJ.GT.L .AND. NMESH.LT.0 ) THEN
         DO I = 1,NMESH,30
            WRITE (*,'(a,3i4,2e12.5)') 'dirac A:',L,IKM(1),I,AP(1,1,I),
     &                                 AQ(1,1,I)
         END DO
         WRITE (*,*) ' '
      END IF
